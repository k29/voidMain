...
#include "comm/protobuf/msg_network_camera.pb.h"
#include <string>
...
CameraNetworkDevice::CameraNetworkDevice()
	...
	, lastSettingChange(0)
{
	std::string host = robot.getConfig().getStrValue("camera.remote.ip", "192.168.1.100");
	int port = 23232; // robot.getConfig().getIntValue("camera.remote.port", 23232);
