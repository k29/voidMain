...
IplImage* CameraImageYUV422::getImageAsYUV(uint16_t newImageWidth, uint16_t newImageHeight) const {
...
IplImage* CameraImageYUV422::getImageAsRGB(uint16_t newImageWidth, uint16_t newImageHeight, const ColorManager* colorMgr, bool bgr, PixelEnhanceMode enhanceMode) const {
	// we do not support scaling up
	if (newImageWidth > imageWidth)
		newImageWidth = imageWidth;
	if (newImageHeight > imageHeight)
	...
	for (uint16_t y=0; y < imageHeight; y += stepY) {
		...
		for(uint16_t x=0; x < imageWidth; x += stepX) {
			...
			if (colorMgr) {
				Color color = colorMgr->getPixelColor(*this, x, y);
...
bool CameraImageYUV422::sendRawImage(Transport *transport, bool compress) const {
	/*
