...
** Note this class does not required camCapture. It is completely independent of any image processing **
Make object of class, call doLocalize() *each* iteration of program
confidence() returns confidence level of current localization
getGoalCoords() returns field coordinates (absolute coords) of specified goal (yellow or blue)
selfX, selfY and selfAngle store current coordinates in centimetres and degrees
...
{
private:
	static const int NO_OF_PARTICLES = 1000;
	...
	static const int MAX_X = 300;	//Maximum width of field in cms
	static const int MAX_Y = 200;	//Maximum height of field in cms
...
public:
	...
	Localize();
	void printPosition();
	void doLocalize(FeatureDetection &fd, MotionModel &mm);
	double confidence();
