...
int main()
{
	...
	compliance(0,10,10);
	state = dribble(flag,lYin,slYin,vYin,vTTar,lZin,slZin,vZin,vZfi);
	
	//lipm_generatorSSP(flag, 0, 0.010, -0.01 ,0.75);
	flag=1-flag;
	
	//	state = dribble(flag,state[YR],state[Y],state[YV],10*2.4,state[ZR],state[Z],-state[ZV],state[ZV]);
		state = dribble(flag,state[YR],state[Y],state[YV],vTTar,state[ZR],state[Z],-state[ZV],state[ZV]);
		state = dribble(flag,state[YR],state[Y],state[YV],vTTar,state[ZR],state[Z],-state[ZV],state[ZV]);
		state = dribble(flag,state[YR],state[Y],state[YV],vTTar,state[ZR],state[Z],-state[ZV],state[ZV]);
		//state = dribble(flag,state[YR],state[Y],state[YV],state[YV]*2.4,state[ZR],state[Z],-state[ZV],state[ZV]);
		//state = dribble(flag,state[YR],state[Y],state[YV],state[YV]*2.4,state[ZR],state[Z],-state[ZV],state[ZV]);
		usleep(16670*2);	
		flag=1-flag;
