...
void set_offset()
{
	if(acyut==4)
	{
		...
		offset[1]=350-25-20; 
		offset[2]=275-10; 
		offset[3]=120+60;//232-20-25-25-10;    // 142s  
		offset[4]=24; 
		offset[5]=-15-40;//+7; 
		offset[6]=346-40; 
		offset[7]=0; 
...
limp_generator(int leg, float legYin, float revlegYin, float legYfi, float revlegYfi,float  float sspTime, float dspTime)
{
	int fps=100;
	int sleep=(int)((float)1000000.0/fps);
	float height = 390;
	float lift   = 40;
	float xfreq  = 2*3.14;
	...
	float startY = asinh(legYin);
	float stopY  = asinh(legYfi);
	float startYR = asinh(revlegYin);
	float stopYR  = asinh(revlegYfi);
...
int main()
{
	...
	limp_generator(flag, 0, 0, 0, 0, 0.75, 0.1);
	flag=1-flag;
	while(!kbhit2())
	{
		limp_generator(flag, 0, 0, 0, 0, 0.75, 0.1);
