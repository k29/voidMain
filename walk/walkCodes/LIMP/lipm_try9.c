...
void dribble(int leg, float legYin, float supLegYin, float veloYin, float veloYfi, float legZin, float supLegZin)
{
	...
	float Tc = 2.0/3*0.95;//0.4;//0.2895;//sqrt(476.0/9810.0);  // Pendulum Constant : Ratio of height of COM plane from Ground plane to acc due to gravity
// 	float Ty = -0.175;               // Y Time Constant : Ratio of COM position form origin (pendulum attachment to ground) along Y axis to velocity of COM in Y direction 
	float Kz = 1;
	float veloRat = veloYfi/veloYin;   // Velocity Ratio  : Ration of final to initial COM velocity in Y direction 
	
	float Tz=-5.00/6*0.95;
	float Tzd=-1*0.95;
	legZin = Tzd*veloZin-hipLength/2;
	supLegZin = Tzd*veloZin-hipLength/2;
	
	printf("LegZin\t%lf\n",legZin);
	///**** Formula used below for Tz calculation is valid only for Kz = 1. To obtain Tz for value other than Kz = 1 resolve using original Sheet  
//	float Tz = (sqrt(pow(Ty,4)-(1-pow(veloRat,2))*pow(Tc,2)*pow(Ty,2) + pow(veloRat,2)*pow(Tc,4) -2*pow(Tc,2)*Ty*veloRat*sqrt(pow(Tc,2)*(pow(veloRat,2)-1)+pow(Ty,2)))+pow(Tc,2)-pow(Ty,2))/(Ty*veloRat-sqrt(pow(Tc,2)*(pow(veloRat,2)-1)+pow(Ty,2)));
	//float sspTime = asinh((-Kz*Tz*Tc - Tz*sqrt((pow(Kz,2)-1)*pow(Tz,2) + pow(Tc,2)))/(pow(Tz,2)-pow(Tc,2)))*Tc;
//	float sspTime = Tc * asinh((sqrt((pow(veloRat,2)-1)*pow(Tc,2) + pow(Ty,2))*Tc - veloRat*Ty*Tc)/(pow(Tc,2)-pow(Ty,2)));
	float sspTime = Tc * asinh((-Tz*sqrt((pow(Kz,2)-1)*pow(Tz,2)+pow(Tc,2))-Kz*Tz*Tc)/(pow(Tz,2)-pow(Tc,2)));
	float m=sinh(sspTime/Tc);
	
	//float Ty=-(Tc*veloRat - Tc*(sqrt(4*m*Tc-4*pow(Tc,2)*sqrt(pow(veloRat,2)-1)+4*pow(m,2)+pow(veloRat,2)-4*m*Tc*sqrt(pow(veloRat,2)-1))))/(2*m+2*Tc);
//	float Ty = Tc*Tc/Tz; Ideal case solution veloRat=1,Kz=1;
	
	float Ty=Tc*(veloRat-sqrt(1+pow(m,2)))/m;
	printf("SSPTime\t%lf\tTc\t%lf\tTy\t%lf\n",sspTime,Tc,Ty);
	
	float dsp1Time = Tz-Tzd;
	float dsp2Time = dsp1Time;//sspTime*dsp2Rat;
	...
	float sspLegYin    = legYin - veloYin*dsp1Time;
//	float sspLegYfi    = (-Tyd+dsp2Time)*veloYfi;
//	float sspSupLegYin = supLegYin - veloYin*dsp2Time;
	float sspSupLegYin = supLegYin - veloYin*dsp1Time;
...
int main()
{
	...
	int flag=0;
	compliance(0,10,10);
	dribble(flag,-0.22*120,0.22*120 ,120, 120,5,5);
	//lipm_generatorSSP(flag, 0, 0.010, -0.01 ,0.75);
	flag=1-flag;
	while(!kbhit2())
	{
		dribble(flag,YYRR,YY ,120, 120,5,5);
//		usleep(16670*2);	
		flag=1-flag;
