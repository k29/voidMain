...
void dribble(int leg, float legYin, float supLegYin, float veloYin, float veloYfi, float legZin, float supLegZin)
{
	...
	float Tc = sqrt(476.0/9810.0);  // Pendulum Constant : Ratio of height of COM plane from Ground plane to acc due to gravity
 	float Ty = -0.160;               // Y Time Constant : Ratio of COM position form origin (pendulum attachment to ground) along Y axis to velocity of COM in Y direction  
	float Kz = 1;
...
int main()
{
	...
	while(!kbhit2())
	{
		dribble(flag, -10, 10, 10/0.20, 10/0.20, -9, -9);
		usleep(16670*2);	
		flag=1-flag;
