...
void dribble(int leg, float legYin, float supLegYin, float veloYin, float veloYfi)
{
	float legZin = 7.5;
	float legZfi = 0;
	...
	//float sspTime2 = asinh((-Kz*Tz*Tc - Tz*sqrt((pow(Kz,2)-1)*pow(Tz,2) + pow(Tc,2)))/(pow(Tz,2)-pow(Tc,2)))*Tc;
	
	//float dsp1Time = supLegYin/veloYin - Ty;   // dsp1Time : Time for dsp1 phase
	
	if(dsp1Time < 0)
	{
		dsp1Time = 0; 
		Ty = supLegYin/veloYin;
	}
	
	float sspYAmp  = veloYin*sqrt(pow(Tc,2)-pow(Ty,2));
	float sspYPhs  = asinh(-Ty/sqrt(pow(Tc,2)-pow(Ty,2)));
	//float sspTime  = (acosh(veloRat/sqrt(pow(Tc,2)-pow(Ty,2)))  - sspYPhs) * Tc ;
	...
	/*float sspZAmp  = sqrt(pow(legZin + hipLength/2,2) - pow(veloZin*Tc,2));
	float sspZPhs  = asinh(Tc * veloZin/sspZAmp);
	float sspZfi   = sspZAmp * cosh(sspTime/Tc + sspZPhs) - hipLength/2;
	...
	*/
	...
	for(walkTime = 0.0; walkTime<=sspTime; walkTime +=timeInc)
	{
		zr = sspZAmp * cosh( walkTime/Tc + sspZPhs) -hipLength/2;
		printf("ZR\t%lf\n",zr);
		///printf("X\t%lf\tXR\t%lf\tZ\t%lf\tZR\t%lf\tY\t%lf\tYR\t%lf\n",x,xr,z,zr,y,yr);
		/*fval=generateik(x,y,z,leg);
