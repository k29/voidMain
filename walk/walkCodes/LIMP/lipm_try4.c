...
enum yMotion{STANDARD, START, STOP, TERMINAL}; 

float dribble(int leg, float legYin, float supLegYin, float veloYin, float veloYfi)
{
	int yMotionState = 0;
	...
	/* LIPM based walk 
	*  Zmax is the weight shift provided for the robot 
	*  time is calculated as the time that will be required to reach Zmax and return to initial value
	*  Y motion is constraint to the this time value  
	
	NOTE : Final implementation will have the time calculated as per the needs of the Y motion and Zmax value calculated based on this time
	       Zmax will hence vary with Y motion
	*/
	
	float pendulumTc = sqrt(476.0/9810.0);
	printf("TC %lf\n\n\n",pendulumTc);
	if(veloYin*pendulumTc<-supLegYin)
	{
		yMotionState = TERMINAL;
		printf("TERMINAL CONDITION REACHED\n");
		exit(0);
	}
	else if(compare_float(supLegYin*pendulumTc, legYin, 0.01) == 1)
	{
		if(compare_float(legYin, 0, 0.01) == 0)
		yMotionState = START;
		else 
		yMotionState = STOP;
	}
	...
	float amplitudeSY = sqrt(pow(veloYin*pendulumTc,2)-pow(sspSupLegYin,2));
	float phaseSY = asinh(sspSupLegYin/amplitudeSY);
	float sspTime = (acosh(veloYfi*pendulumTc/amplitudeSY)-phaseSY)*pendulumTc;
	
	
	float sspSupLegYfi = amplitudeSY*sinh(sspTime/pendulumTc+phaseSY);
	float sspLegYfi    = -sspSupLegYfi - 2*veloYfi*dsp2Time;
		
	float stepTime = sspTime + dsp1Time + dsp2Time;
	float zmax = (hipLength/2 +feetSeperation) * (1 - 1/cosh(stepTime/(2*pendulumTc))) ;
	
	//float time = 2*acosh(hipLength/(2*(hipLength/2-zmax)))*pendulumTc;	
	float legYfi = -sspLegYfi - veloYfi * dsp2Time; 
	
	
	printf("AMP\t%lf\tPHASE\t%lf\tsspTIME\t%lf\tTIME\t%lf\tZmax\t%lf\tSYfi%lf\n",amplitudeSY,phaseSY,sspTime,stepTime,zmax,-sspSupLegYfi);
	...
	if(yMotionState == START)
	{
	//	sspTime = ln(veloYfi)
	}
	
	////// Single Support Phase ///////
	for(walkTime = 0.0; walkTime - 2 * timeInc<=stepTime; walkTime +=timeInc)
	{
		if(walkTime  - timeInc <= dsp1Time)
		{
			printf("D1\t");
			x  = height;
			xr = height;
			
			//zr = (hipLength/2-zmax) * cosh(walkTime/pendulumTc - acosh(hipLength/(2*(hipLength/2-zmax)))) - hipLength/2 + feetSeperation;
			
			zr = (hipLength/2 +feetSeperation-zmax) * cosh(walkTime/pendulumTc - acosh( (hipLength +2 *feetSeperation)/(2*(hipLength/2 + feetSeperation-zmax)))) - (hipLength/2);
			z  = feetSeperation;
			
			if(yMotionState	== STANDARD)
			{
				y  =  legYin - veloYin * walkTime;
				yr =  supLegYin - veloYin * walkTime;
			}
			else if(yMotionState == START)
			{
//				yr = 
			}
			
		}
		else if(walkTime - timeInc <= sspTime + dsp1Time)
		{
			printf("S \t");
			...
			if(yMotionState	== STANDARD)
			{
				y  = linear(-sspLegYin, -sspLegYfi, walkTime - dsp1Time, sspTime);///amplitudeSY*sinh(walkTime/pendulumTc+phaseSY);
				yr = -(amplitudeSY*sinh((walkTime-dsp1Time)/pendulumTc+phaseSY));
			}
			else if(yMotionState == START)
			{
		...
		else if(walkTime - timeInc <= sspTime + dsp1Time + dsp2Time)
		{
			printf("D2\t");
			x  = height;
			xr = height;
			...
			if(yMotionState	== STANDARD)
			{
				y  =  -sspLegYfi - veloYfi * (walkTime - sspTime - dsp1Time);
				yr =  -sspSupLegYfi - veloYfi * (walkTime - sspTime - dsp1Time) ;
			}
			else if(yMotionState == START)
			{
				y  = 0;
		...
		printf("T\t%3.3lf\tX\t%3.3lf\tXR\t%3.3lf\tZ\t%3.3lf\tZR\t%3.3lf\tY\t%3.3lf\tYR\t%3.3lf\n",walkTime,x,xr,z,zr,y,yr);
		fval=generateik(x,y,z,leg);
...
int main()
{
	...
	while(!kbhit2())
	{
		dribble(flag,-30,30,150,150);
	//	dribble(flag,-20,20,150,150);
		flag=1-flag;
