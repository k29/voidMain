...
void dribble(int leg, float legYin, float supLegYin, float veloYin, float veloYfi, float legZin, float supLegZin)
{
	...
	float sspLegZfi    = veloZfi*(-Tz+2*dsp1Time) - hipLength/2;
...
int main()
{
	...
	*/
	float *fval;
	fval = generateik(180+180+49,0,0,0);
	int test_i=0;
	for(test_i=0;test_i<5;test_i++)
	printf("%d\t%d\n",test_i,(int)fval[test_i]+(test_i==4?offset[test_i+1]:offset[test_i]));
