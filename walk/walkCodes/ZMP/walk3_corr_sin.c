...
/*float pid_z(int leg)
{
	...
	for(k=0;k<future_size;k++)
	{
		...
		future_err_z[k]=0;
		for(j=0;j<size;j++)
		future_err_z[k]+=prev_err_z[k][j][0]/(j+1);
		printf("%lf\n",future_err_z[k]);
	}
...
int walk3(int leg,float yin,float yfi,float yrin,float yrfi, float zin, float zfi,float zrin, float zrfi, float thetain, float thetafi,float thetarin, float thetarfi)
{
	...
	/////// X-Land Control /////
	//float dec_time=0.75;	
	float x_land_window=0.15;//1-x_end    
	float x_land_dec=0;
	float x_land_freq=2*3.14;
	float x_land_displacement=1;
	float x_land_phase=-1.57;

	/////// X-Load Control /////
	float x_load_start=0;
	float x_load_stay=0.0625;
	float x_load_increase_end=0.0625;
	float x_load_end=0.125;
	float x_load_displacement=1;
	float x_load_phase=-1.57;
	float x_load_freq=3.14;
	float x_load_dec=0.5;
	//float x_load_window=0.05;
	
	/////// Z-Shift Control /////
	float peak_start=0.0;
	float peak_max=0.2;//0.5
	float peak_stay=0.2;//0.75
	float peak_end=0.8;
	float zmax=25;//*leg;
...
//	float peaktime=0.75;
	...
	for(frame=0;frame<time*fps;frame++,t+=1.0/fps)
	{
		...
		///// Soften landing     
		if(t_frac>x_end-x_land_window&&t_frac<x_end+x_land_window)
		...
		///// Assist weight shift
		if(t_frac>x_load_start&&t_frac<=x_load_stay)
		xr-=x_load_dec*(sin(x_load_freq*(t_frac-x_load_start)/(x_load_stay-x_load_start)+x_load_phase)+x_load_displacement)/(1+x_load_displacement);
		else if(t_frac>x_load_stay&&t_frac<=x_load_increase_end)
		xr-=x_load_dec;
		else if(t_frac>x_load_increase_end&&t_frac<=x_load_end)
		xr-=x_load_dec*(1-(sin(x_load_freq*(t_frac-x_load_increase_end)/(x_load_end-x_load_increase_end)+x_load_phase)+x_load_displacement)/(1+x_load_displacement));
		else
		xr=height;
