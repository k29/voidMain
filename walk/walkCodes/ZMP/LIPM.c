...
inline int walk3(int leg,float yin,float yfi,float yrin,float yrfi, float zin, float zfi,float zrin, float zrfi, float thetain, float thetafi,float thetarin, float thetarfi)
{
	...
	/////// X-Load Control /////
	float x_load_start=0;
	float x_load_increase_end=0.25;
	float x_load_decrease_start=0.25;
	float x_load_end=0.5;
	float x_load_displacement=1;//x_land_displacement;
	...
	/////// Z-Shift Control /////
	float peak_start=0.4;
	//float peak_max=1;//0.5
	float peak_stay_start=0.8;//0.75
	float peak_end=1;//0.8
	...
	float sep_freq=3.14;
	//float zmax=25;//*leg;
	float zfreq=1.57;//3.14
	float zphase=0;//-1.57
	float zdisplacement=0;//1
//	float peaktime=0.75;
	
	/////// Z - Land Control /////
	float z_land_trans=0;
	float z_land_window=0.2;//1-peak_end;
	float z_land_freq=2*3.14;
	float z_land_displacement=1;
	float z_land_phase=-1.57;

	/////// Y Control /////
	float y_start=0.05;
	float y_end=0.40;//0.5
	float y1_start=0.40;
	float y1_stay_start=0.6;
	float y1_end=1;
	
	float yr_start=y1_start;
	float yr_stay_start=y1_stay_start;
	float yr_end=y1_end;
	...
	float z_com_trans=7;
	float z_com_window=1-z_com_end;
	float z_com_freq=2*3.14;
	float z_com_displacement=1;
	float z_com_phase=-1.57;
	...
	for(frame=0;frame<time*fps;frame++,t+=1.0/fps)
	{
		...
		z_shift=zmax;
		else if(t_frac>=peak_start&&t_frac<peak_stay_start)
		{ z_shift=-zmax*(cosh(k1*(t_frac-peak_start)/(peak_stay_start-peak_start))-2);
		  printf("zshift %f cosh: %lf\n",z_shift,cosh(k1*(t_frac-peak_start)/(peak_stay_start-peak_start))-2);
		}
		else if(t_frac>=peak_stay_start&&t_frac<peak_end)
		z_shift=-zmax;
		...
		if(t_frac<sep_start)
			seperationr=seperation;
		
		if(t_frac>=sep_start&&t_frac<sep_trans)
		{	seperationr=(sep_max-seperation)*(sin(sep_freq*(t_frac-sep_start)/(sep_trans-sep_start)))+seperation;
		 	printf("seperationr: %f",seperationr);
		}
		
		else if(t_frac>=sep_trans&&t_frac<=sep_end)
		{	seperationr=scurve2(seperation,0,t_frac-sep_trans,sep_end-sep_trans);
		 	printf("seperationr: %f",seperationr);
		}
		...
		seperationrr=scurve2(0,seperation,t_frac-sep_trans,sep_end-sep_trans);
		else
		seperationrr=seperation;
		
		//if(t_frac>peak_end-z_land_window&&t_frac<peak_end+z_land_window)
		//z_shift+=z_land_trans*((sin(z_land_freq*(t_frac-peak_end+z_land_window)/(2*z_land_window)+z_land_phase)+z_land_displacement)/(1+z_land_displacement));
		
		printf("1  zshift %f cosh: %lf\n",z_shift,cosh(k1*(t_frac-peak_start)/(peak_end-peak_start))-2);
		////Y MOVEMENT////
		if(t_frac<y_start)
		y=yin;  
                else if(t_frac>=y_start&&t_frac<y_end)
		y=yin*(1-sinh(0.8813*(t_frac-y_start)/(y_end-y_start)))-yrfi*sinh(0.8813*(t_frac-y_start)/(y_end-y_start));
		else if(t_frac>=y_end&&t_frac<y1_start)
		y=-yrfi;
		else if(t_frac>=y1_start&&t_frac<y1_stay_start)
		y=scurve2(-yrfi,0,t_frac-y1_start,y1_stay_start-y1_start);
		else if(t_frac>=y1_stay_start&&t_frac<=y1_end)
		y=0;
		else 
		y=0;
		
		printf("2  zshift %f cosh: %lf\n",z_shift,cosh(k1*(t_frac-peak_start)/(peak_end-peak_start))-2);
		/////Y MOVT/////

                if(t_frac<yr_start)
		yr=yrin;
		else if(t_frac>=yr_start&&t_frac<yr_stay_start)
		yr=scurve2(yrin,yrfi,t_frac-yr_start,yr_stay_start-yr_start);
		else if(t_frac>=yr_stay_start&&t_frac<yr_end) 
		yr=yrfi;
		else 
		yr=yrfi;
	...
	//////////////// Z MOVEMENT///
		...
		*/
		...
		printf("Z-new %f\n\n",zreal);
		printf("Zr-new %f\n\n",zrreal);
		fval=generateik(x,yreal,zreal+seperationr,leg);
		for(i=0;i<5;i++)
		{	val[i]=fval[i];
		 	printf("\n\n val: %f \n" ,val[i]);		
		}
		fval=generateik(xr,yrreal,zrreal+seperationrr,1-leg);
...
void initialize_acyut()
{
	...
	float *fval;
	int value;
	fval=generateik(390,-STEP,zmax+seperation,0);
	for(h=0;h<5;h++)
	val[h]=fval[h];
	fval=generateik(390,0,-zmax,0);
	for(h=0;h<5;h++)
	valr[h]=fval[h];
...
int main()
{
	...
	{
		//printf("%d\n",read_position(5));
		walk3(flag,-STEP,0,0,-STEP,0,0,0,0,0,0,0,0);
		flag=1-flag;
		usleep(16670*15);
