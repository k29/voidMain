...
#define serialusb2d "A700eSSZ" //"A600cBa7" //"A600cURy"  //"A900fC5U"//nd USB2D: A7003N1d  A7003NDp	 A7005LC8	  A700eST2
...
//	byte buffer[59];
	...
	if(flag>=1)
	{
		for(k=0; k<3; k++)
		{
			...
			printf("AVG_ANGLE %d : %lf     ",k,avg_angle[k]);
			printf("AVG_ACC %d : %lf     ",k,avg_acc[k]);
			printf("AVG_GYRO %d : %lf     ",k,avg_gyro[k]);
		}
...
inline void imucal()
{
			...
			if(iread(IMU_DATA,&imu_t,&imu,&im[0],1)>0)
			{
				...
				if(start==2)
				{
					...
					{	
						...
						//avg_acc[0]+=(im[6]+im[7]*256)-base_acc[0];
						//avg_acc[1]=avg_acc[1]+im[7];
						//avg_acc[2]=avg_acc[2]+im[8];
						for(j=0,k=0;j<6;j+=2,k+=1)
						{
							acc[k]=im[j+6]+im[j+7]*256;
							//acc[k]-=base_acc[k];	
							avg_acc[k]=avg_acc[k]+acc[k]-base_acc[k];
						}
						for(j=0,k=0;j<6;j+=2,k+=1)
						{
							gyro[k]=im[j+12]+im[j+13]*256;
							//acc[k]-=base_acc[k];	
							avg_gyro[k]=avg_gyro[k]+gyro[k]-base_gyro[k];
						}
	...
	{
		//printf("%ld\t",COUNTER++);
		angle_moved+=abs(avg_gyro[0])<200?avg_gyro[0]:0;
		for(k=0;k<3;k++)		
		{
			avg_angle[k]=avg_angle[k]/count;
			avg_acc[k]=avg_acc[k]/count;
			avg_gyro[k]=avg_gyro[k]/count;
			//printf("ANGLE %d : %lf\n",k,avg_angle[k]);
			//printf("ACC %d : %lf\n",k,avg_acc[k]);
			//printf("GYRO %d : %lf\n",k,avg_gyro[k]);
		}
