...
float pid_z(int leg)
{
	...
	cor_z=pk*err_z+pi*sum_err_z+pd*(err_z-prev_err_z[0]);
	if(abs(cor_z)>50)
	{
		printf("*******************************************************************\n");
...
//	float peaktime=0.75;
	...
	float arm_start=0.1;
	// float arm_trans=0.55;
	float arm_end=1;
	float arm_freq=2*3.14;
	float arm_displacement=1;	
	float arm_phase=-1.57;
	float arm_max=170.5;
	float arm_theta=0;    
    ////Value for theta=2(asin(STEP))    
        ///////Separation control////
        float seperation=5;
	...
	for(frame=0;frame<time*fps;frame++,t+=1.0/fps)
	{
		...
		//if(t_frac>arm_start&&t_frac<arm_end)
		{
			arm_theta=(1023.0/270.0)*((180/3.14)*(asin(y/STEP)))/5;
			printf("arm_theta%f\n",arm_theta);
		}
		...
		*/
		...
		//pid_z(leg);
		//spline_prediction();
		// zrreal+=cor_z;//0.5*sum_err(cor_z,1);//sum_sin();
		// zreal-=cor_z;//0.5*sum_err(cor_z,1);//sum_sin();
		...
		for(i=0;i<NOM;i++)
		{
			...
			else if(tx_packet[7+i*3]==mot+6)
			{
				value=arm_theta*pow(-1,leg)+512+offset[tx_packet[7+i*3]];
				if(value>=0&&value<=1024)
				{
					tx_packet[8+i*3]=value%256;
					tx_packet[9+i*3]=value/256;
			...
			else if(tx_packet[7+i*3]==revmot+6)
			{
				value=arm_theta*pow(-1,leg)+512+offset[tx_packet[7+i*3]];
				if(value>=0 && value<=1024)
				{
	...
	for(;t<time;t+=(float)1/60)
	{
		...
		*/
		...
		for(i=0;i<NOM;i++)
		{
			...
			if(tx_packet[7+i*3]==revmot+6)
			{
				value=tx_packet[8+i*3]+tx_packet[9+i*3]*256;
				tx_packet[adchksum]+=tx_packet[8+i*3]+tx_packet[9+i*3];
				//value+=pow(-1,leg)*chr*1023/300;
				value=512+offset[tx_packet[7+i*3]];
				if(value>=0 && value<=1023)
				{
					tx_packet[8+i*3]=value%256;
					tx_packet[9+i*3]=value/256;
			...
			if(tx_packet[7+i*3]==revmot+6)
			{
				...
				tx_packet[adchksum]+=tx_packet[8+i*3]+tx_packet[9+i*3];
				//value+=pow(-1,leg)*chr*1023/300;
				value=yreal+512+offset[tx_packet[7+i*3]];
				if(value>=0 && value<=1023)
				{
					tx_packet[8+i*3]=value%256;
					tx_packet[9+i*3]=value/256;
