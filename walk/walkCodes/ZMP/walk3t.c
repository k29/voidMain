...
inline int walk3(int leg,float yin,float yfi,float yrin,float yrfi, float zin, float zfi,float zrin, float zrfi, float thetain, float thetafi,float thetarin, float thetarfi)
{
	...
	/////// X-Load Control /////
	float x_load_start=-1;
	float x_load_increase_end=-0.5;
	float x_load_decrease_start=0;
	...
	float peak_max=0.2;//0.5
	float peak_stay=0.6;//0.75
	float peak_end=0.8;
	float zmax=25;//*leg;
...
//	float peaktime=0.75;
	...
	float z_land_window=1-peak_end;
	...
	//Z Movement
	float z_com_start=0;
	float z_com_peak=0.2;
	float z_com_stay=0.6;
	float z_com_end=0.8;
	float z_com_max=25;
	
	float z_com_trans=15;
	...
	for(frame=0;frame<time*fps;frame++,t+=1.0/fps)
	{
		...
		else if(t_frac>peak_max&&t_frac<peak_stay)
		z_shift=z_shift;
		...
		com_z=z_com_max-z_com_max*sin(1.57*(t_frac-z_com_stay)/(z_com_end-z_com_stay));//+1.57);
		else
		com_z=0;
		...
		*/
		...
		printf("z_com_start %f z_com_peak %f z_com_stay %f,z_com_end %f ,z_com_max %f,z_com_trans %f ,z_com_window %f,z_com_freq %f ,z_com_displacement %f ,z_com_phase%f\n",z_com_start ,z_com_peak,z_com_stay,z_com_end,z_com_max,z_com_trans,z_com_window,z_com_freq,z_com_displacement,z_com_phase);
		printf("peak_start %f peak_max %f ,peak_stay %f ,peak_end %f ,zmax %f ,zfreq %f ,zphase %f ,zdisplacement %f\n",peak_start,peak_max,peak_stay,peak_end,zmax,zfreq,zphase,zdisplacement);

		printf("Z_SHIFT : %lf X : %lf XR : %lf\n",z_shift,x,xr);
		zreal+=z_shift;
		zrreal-=z_shift;
		/*if(abs(err_z)<25)
		{
		...
		pid_z(leg);
		//spline_prediction();
		//zrreal+=sum_sin();//0.5*sum_err(cor_z,1);
		//zreal-=sum_sin();//0.5*sum_err(cor_z,1);
