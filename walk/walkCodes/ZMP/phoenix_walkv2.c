...
#define COM_STAND sqrt(LLENGTH*LLENGTH-HSTEP*HSTEP-seperation*seperation)-1 //406
#define COM_TRANS sqrt(LLENGTH*LLENGTH-4*HSTEP*HSTEP-(2*seperation+HPLATE)*(2*seperation+HPLATE))
#define COM_LOW   360
#define LEG_HIGH 300//290
#define LSWAY 0
...
float sum_err(float *err, float coeff_a)
{
	...
	{
	per=err[i]*i*i*pow(E,-coeff_a*i);
	sum+=pow(E,-coeff_a*i)*err[i];
	//printf("Actual error:%f ERR per added=%f\n\n",err[i],per);
	}
...
/*float pid_z(int leg)
{
	...
	{
		for(i=size;i>0;i--)
		prev_err_z[k][0][i]=k==0?prev_err_z[k][0][i-1]:prev_err_z[k-1][0][i-1];
		prev_err_z[k][0][0]=k==0?err_z:future_err_z[k-1];
		
		for(j=1;j<size;j++)
		{
			for(i=0;i<size-j;i++)
			prev_err_z[k][j][i]=(prev_err_z[k][j-1][i]-prev_err_z[k][j-1][i+1]);
		}
		...
		{
			for(j=0;j<size;j++)
			printf("%lf\t",prev_err_z[k][i][j]);
			printf("\n");
		}
		
		exit=0;
		future_err_z[k]=0;
		for(j=0;j<size&&exit==0;j++)
		{
			//if(j<size-1&&fabs(prev_err_z[k][j+1][0])>fabs(prev_err_z[k][j][0]))
			//exit=1;
			future_err_z[k]+=prev_err_z[k][j][0];
			
		}
		//future_err_z[k]*=exp(-2*k);
		printf("%lf\n",future_err_z[k]);
	}			
}*/
...
/*float sum_sin()
{
	int i=size,exit=0;
	float ret=0;
	for(i=0;i<future_size&&exit==0;i++)
	{
		
		if (fabs(future_err_z[i+1])<4000)
		exit=1;
		//ret+=cor_z[i]*sin((float)(i+1)/(float)size*3.14);
		ret+=future_err_z[i]*exp(-i*i);//*sin((float)(i+1)/(float)future_size*3.14)*exp(-i*i);	
	}
	printf("CORRECTION %lf\n",0.21*ret);//future_size);
	return 0.21*ret;//future_size;
}
float prev_err_z[120]={0};
*/
inline int walk3(int leg,float yin,float yfi,float yrin,float yrfi, float zin, float zfi,float zrin, float zrfi, float thetain, float thetafi,float thetarin, float thetarfi)
{
	// /*FILE *f;
 ...
 // 	/////COMPLIANCE//
	...
	for(frame=0;frame<time*fps;frame++,t+=1.0/fps)
	{
		...
		////X LIFT////		
		if(t_frac>=x_start&&t_frac<x_trans)
		{
			x=(scurve2(COM_STAND,COM_TRANS,t_frac-x_start,x_trans-x_start));
			xr=(scurve2(COM_STAND,COM_TRANS,t_frac-x_start,x_trans-x_start));
			
			printf("1. %f\n",x);

		}
		else if(t_frac>=x_trans&&t_frac<x_end)
		{
			//xr=(scurve2(COM_TRANS,COM_,t_frac-x_trans,x_trans-x_start));
			x=(scurve2(COM_TRANS,LEG_HIGH,t_frac-x_trans,x_end-x_trans));
			xr=(scurve2(COM_TRANS,COM_LOW,t_frac-x_trans,x_end-x_trans));
			printf("2. %f\n",x);
		}

		if(t_frac>=x2_start&&t_frac<x2_trans)
		{
			x=(scurve2(LEG_HIGH,COM_TRANS,t_frac-x2_start,x2_trans-x2_start));
			xr=(scurve2(COM_LOW,COM_TRANS,t_frac-x2_start,x2_trans-x2_start));
			printf("3. %f\n",x);
		}
		else if(t_frac>=x2_trans&&t_frac<=x2_end)
		{
			//xr=(scurve2(COM_TRANS,COM_,t_frac-x_trans,x_trans-x_start));
			x=(scurve2(COM_TRANS,COM_STAND,t_frac-x2_trans,x2_end-x2_trans));
			xr=(scurve2(COM_TRANS,COM_STAND,t_frac-x2_trans,x2_end-x2_trans));
			printf("4. %f\n",x);
		}
        ...
        else if(t_frac>=y_start&&t_frac<y_trans)
		{
			y=scurve2(-HSTEP,-2*HSTEP,t_frac-y_start,y_trans-y_start);
			yr=scurve2(HSTEP,0,t_frac-y_start,y_trans-y_start);
		}
		else if(t_frac>=y_trans&&t_frac<y_end)
		{
			y=scurve2(-2*HSTEP,0,t_frac-y_trans,y_end-y_trans);
			yr=scurve2(0,0,t_frac-y_trans,y_end-y_trans);
		}
		
		else if(t_frac>=y2_start&&t_frac<y2_trans)
		{
			y=scurve2(0,2*HSTEP,t_frac-y2_start,y2_trans-y2_start);
			yr=scurve2(0,0,t_frac-y2_start,y2_trans-y2_start);
		}
		else if(t_frac>=y2_trans&&t_frac<y2_end)
		{
			y=scurve2(2*HSTEP,HSTEP,t_frac-y2_trans,y2_end-y2_trans);
			yr=scurve2(0,-HSTEP,t_frac-y2_trans,y2_end-y2_trans);
		}
		...
		else if(t_frac>=z_start&&t_frac<z_trans)	
	    {
			z=scurve2(seperation,seperation*2+HPLATE,t_frac-z_start,z_trans-z_start);
			zr=scurve2(seperation,-HPLATE,t_frac-z_start,z_trans-z_start);
		}
		else if(t_frac>=z_trans&&t_frac<z_end)
		{
 
			z=scurve2(seperation*2+HPLATE,seperation*2+HSUP+LSWAY,t_frac-z_trans,z_end-z_trans);
			zr=scurve2(-HPLATE,-HSUP,t_frac-z_trans,z_end-z_trans);
		}
		else if(t_frac>=z2_start&&t_frac<z2_trans)	
	    {
			z=scurve2(seperation*2+HSUP+LSWAY,seperation*2+HPLATE,t_frac-z2_start,z2_trans-z2_start);
			zr=scurve2(-HSUP,-HPLATE,t_frac-z2_start,z2_trans-z2_start);
		}
		else if(t_frac>=z2_trans&&t_frac<z2_end)
		{
 
			z=scurve2(seperation*2+HPLATE,seperation,t_frac-z2_trans,z2_end-z2_trans);
			zr=scurve2(-HPLATE,seperation,t_frac-z2_trans,z2_end-z2_trans);
		}
		else 
		{
			z=0; zr=0;
		...
		{
			phi=scurve2(thetain,thetafi,t_frac-turn_start,turn_end-turn_start);
			phir=scurve2(thetarin,thetarfi,t_frac-turn_start,turn_end-turn_start);
		}
		else
		{
			phi=thetafi;
		...
		printf("X : %f XR: %f Z : %f ZR %f ",x,xr,z,zr);
		printf("Y : %f YR %f \n", y,yr);
		/////// COM Trajectory /////
		///////// Z Axis ////////
		...
		// //if(t_frac>peak_end-z_land_window&&t_frac<peak_end+z_land_window)
		// //com_z+=z_com_trans*(sin(z_com_freq*(t_frac-peak_end+z_com_window)/(2*z_com_window)+z_com_phase)+z_com_displacement)/(1+z_com_displacement);
		// if(frame!=0)
...
//			printf("valr:  %f\n\n",valr[i]);
		...
		target_left[5]=val[4]*(1-leg)+valr[4]*leg+cor_z*(1-leg);
		...
		target_right[5]=valr[4]*(1-leg)+val[4]*leg+cor_z*(leg);
...
{	
	...
	for(;t<time;t+=(float)1/60)
	{
		...
		z//_shift=min(zmax,zfreq*(sin(2*3.14*t_frac-1.57)+1)/2);
...
void initialize_acyut()
{
	...
	for(i_init=0;i_init<NOM;i_init++)
	{
		...
		if(tx_packet[7+i_init*3]==20)
		{
			value=valr[0]+offset[tx_packet[7+i_init*3]];
			tx_packet[8+i_init*3]=value%256;
			tx_packet[9+i_init*3]=value/256;
			//printf("value %d \t %d\n",tx_packet[7+i_init*3],value);
		}
		if(tx_packet[7+i_init*3]==1)
		{
		...
		if(tx_packet[7+i_init*3]==21)
		{
			value=valr[1]+offset[tx_packet[7+i_init*3]];
			tx_packet[8+i_init*3]=value%256;
			tx_packet[9+i_init*3]=value/256;
			//printf("value %d \t %d\n",tx_packet[7+i_init*3],value);
		}
		if(tx_packet[7+i_init*3]==2||tx_packet[7+i_init*3]==12)
		{
		...
		if(tx_packet[7+i_init*3]==22||tx_packet[7+i_init*3]==32)
		{
		...
		if(tx_packet[7+i_init*3]==23)
		{
			value=valr[3]+offset[tx_packet[7+i_init*3]];
			tx_packet[8+i_init*3]=value%256;
			tx_packet[9+i_init*3]=value/256;
			//printf("value %d \t %d\n",tx_packet[7+i_init*3],value);
		}
		if(tx_packet[7+i_init*3]==5)
		{
		...
		if(tx_packet[7+i_init*3]==25)
		{
			value=valr[4]+offset[tx_packet[7+i_init*3]];
			tx_packet[8+i_init*3]=value%256;
			tx_packet[9+i_init*3]=value/256;
			//printf("value %d \t %d\n",tx_packet[7+i_init*3],value);
		}
...
int main()
{
	...
	initialize_acyut();
	//imucal_init();
	walk3(1-flag,-HSTEP,HSTEP,HSTEP,-HSTEP,0,0,0,0,0,0,0,0);
	usleep(16670*10);
	
	while(!kbhit2())
	{
		//printf("%d\n",read_position(5));
		walk3(flag,-HSTEP,HSTEP,HSTEP,-HSTEP,0,0,0,0,0,0,0,0);
		flag=1-flag;
		usleep(16670*15);
		//projection_system(flag);
		
	}
	//walk3(flag,-10,0,STEP,0,0,0,0,0,0,0,0,0);
	usleep(16670*30);
	//compliance(0,32,32);
