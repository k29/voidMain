...
float pid_z(int leg)
{
	...
	for(i=size;i>0;i--)
	prev_err_z[0][0][i]=prev_err_z[0][0][i-1];
	prev_err_z[0][0][0]=err_z;
	
	for(i=0;i<future_size;i++)
	{
		for(j=size;j>0&&i!=0;j--)
		{
			//printf("KKKKK\n");
			prev_err_z[i][0][j]=prev_err_z[i-1][0][j-1];
		}		
		for(j=1;j<size;j++)
		{
			for(k=0;k<size-j;k++)
			prev_err_z[i][j][k]=(prev_err_z[i][j-1][k]-prev_err_z[i][j-1][k+1])/2;
		}
		prev_err_z[i+1][0][0]=0;
		float val=0;
		for(j=0;j<size;j++)
		val+=prev_err_z[i][j][0];
		printf("%lf\n",val);
	}
