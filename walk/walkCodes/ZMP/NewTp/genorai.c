...
{
	result[0]=cos(ang[1])*quan[0]+sin(ang[1])*sin(ang[0])*quan[1]-sin(ang[1])*cos(ang[0])*quan[2];
...
//			printf("ERR_Z %12.8f ",err_z);//printf("ERR_Y %3.7f\tERR_Z %3.7f\t%3.7f\t%3.7f\t",err_y,err_z,(rot_hip[1]+(-50)*(1-leg)+50*leg)*pow(-1,leg+1),com_z);
			...
			seq_crit=(rot_hip[1]*pow(-1,leg)-pow(hip_velo[1]*1000,2)*signum(hip_velo[1])/10000);
	...
	//	printf("SigCZR %2d ",signum(-diff_shift*cor_zr[sequence]));
		...
		cor_zr[sequence]+=0.75*(-hip_velo[1]*1000-diff_shift);
...
int main()
{
	bootup_files();
	imu_initialize();
	float set_point[]={10,0,0};
	float ans[3];
	while(1)
	{
		get_imu_data();
		rotateToIMU(set_point,ans);
		printf("%10.5f %10.5f %10.5f\n",ans[0],ans[1],ans[2]);
