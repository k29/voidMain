...
float left_com[3]={0};
float right_com[3]={0};
float com[3]={0},rot_com[3]={0},rot_hip[3]={0},hip[3]={0};
int offset[33]={0};
int motor_val[33]={0};
int target_left[6];
int target_right[6];
...
int  forward_kinematics(int leg,int motor_val[6],float coods[][3])   
{
	...
	for(i=0;i<6;i++)
	{
		...
		//printf("%f\t%f\t%f\n",coods[i+1][0],coods[i+1][1],coods[i+1][2]);
...
int generate_leg_com(int leg, float coods[][3],float *leg_com)
{	
	int i=0,j=0;
...
int generate_com(float leftLeg[][3],float rightLeg[][3],float *com,float *left_com,float *right_com)
{
	generate_leg_com(0,leftLeg,left_com);
	generate_leg_com(1,rightLeg,right_com);
...
int rotate_com(int leg,float *com,float *leftLeg,float *rightLeg,float *rot_com,float *rot_leftLeg,float *rot_rightLeg)
{	
	FILE *f;
...
//	float coods[]={-com_leg_frame[1],com_leg_frame[0],-com_leg_frame[0]}
	...
	rotateToIMU(rightLeg,rot_rightLeg);
	//rot_hip[1]+=(-50)*(1-leg)+50*leg;

	rotateToIMU(leftLeg,rot_leftLeg);
	//rot_rleg[1]+=(-50)*(1-leg)+50*leg;
	
	
	if(inRange(rot_rightLeg[0]-rot_leftLeg[0],-3,3)==1)
	{
		grounded=9;
/*		if(leg==0)*/
/*		{*/
/*			rot_hip[0]=-rot_leftLeg[0];*/
/*			rot_hip[1]=-rot_leftLeg[1]-50;*/
/*			rot_hip[2]=-rot_leftLeg[2];		*/
/*			rot_com[0]=rot_com[0]-rot_leftLeg[0];*/
/*			rot_com[1]=rot_com[1]-rot_leftLeg[1]-50;*/
/*			rot_com[2]=rot_com[2]-rot_leftLeg[2];*/
/*		}*/
/*		else if(leg==1)*/
/*		{*/
/*			rot_hip[0]=-rot_rightLeg[0];*/
/*			rot_hip[1]=-rot_rightLeg[1]+50;*/
/*			rot_hip[2]=-rot_rightLeg[2];*/
/*			rot_com[0]=rot_com[0]-rot_rightLeg[0];*/
/*			rot_com[1]=rot_com[1]-rot_rightLeg[1]+50;*/
/*			rot_com[2]=rot_com[2]-rot_rightLeg[2];*/
/*		}*/
	}
	else if(rot_rightLeg[0]>rot_leftLeg[0])
	{
		grounded=0;
/*		rot_hip[0]=-rot_leftLeg[0];*/
/*		rot_hip[1]=-rot_leftLeg[1]-50;*/
...
/*		rot_com[2]=rot_com[2]-rot_rightLeg[2];*/
	...
	if(leg==0)
	{
		rot_rleg[0]=rot_rightLeg[0]-rot_leftLeg[0];
		rot_rleg[1]=rot_rightLeg[1]-rot_leftLeg[1];
		rot_rleg[2]=rot_rightLeg[2]-rot_leftLeg[2];
	}
	else if (leg==1)
	{
		rot_rleg[0]=rot_leftLeg[0]-rot_rightLeg[0];
		rot_rleg[1]=rot_leftLeg[1]-rot_rightLeg[1];
		rot_rleg[2]=rot_leftLeg[2]-rot_rightLeg[2];
...
int  projection_system(int leg)
{
	...
	*/
	...
	forward_kinematics(0,val,left_joints);
	//for(i=0;i<7;i++)
	//printf("LEFT  %f\t%f\t%f\n",left_joints[i][0],left_joints[i][1],left_joints[i][2]);
	...
	forward_kinematics(1,val,right_joints);
	//for(i=0;i<7;i++)
	//printf("RIGHT  %f\t%f\t%f\n",right_joints[i][0],right_joints[i][1],right_joints[i][2]);
	
	
	generate_com(left_joints,right_joints,com,left_com,right_com);
...
int setupWalk(int leg)
{
	...
	proposedZShift+=pow(hip_velo[1]*1000,2)*signum(hip_velo[1]*pow(-1,leg+1))/(19000*fabs(cos(current_angle[1]*3.14/180)));
	...
	if(proposedZShift>45)
	proposedZShift=45;
	else if(proposedZShift<0)
	proposedZShift=0;
	
	//walk.left.z_shift.zmax=47;
	
	//init_phase=asin(((prevWalk.set_point[0][2]+prevWalk.set_point[1][2])*pow(-1,leg+1)/2)*(1+walk.left.z_shift.displacement)/walk.left.z_shift.zmax-walk.left.z_shift.displacement)-setPhase;
	printf("VELO %10.5f Zmax %10.5f\n",hip_velo[1],proposedZShift);
	...
	walk.left.z_shift.zmax=0.5*(proposedZShift+prevWalk.zmax);
	walk.left.z_shift.startPhase=-PI/2;//init_phase;
	...
	walk.left.z_shift.displacement=(proposedZShift-prevWalk.zmax)/(2*walk.left.z_shift.zmax);//1;
...
float setWalkShift(float t_frac,walk_zshift zshift, int phase)
{
	...
	{
		printf("HER %10.5f\n",zshift.zmax);
		zshift_val=zshift.zmax*(sin((zshift.stayPhase-zshift.endPhase)*(t_frac-zshift.stayTime)/(zshift.endTime-zshift.stayTime)+zshift.stayPhase)+zshift.displacement)/(1+zshift.displacement)  +   zshift.decAmp*(sin(zshift.decFreq*((t_frac-zshift.stayTime)/zshift.decTime)+zshift.decPhase)+zshift.decDisplacement)/(1+zshift.decDisplacement);
	}
...
float predictAlpha(float *com)
{
	float feetDimension[2][2];
	feetDimension[0][0]=0;
	feetDimension[0][1]=0;
	feetDimension[1][0]=45;
	feetDimension[1][1]=55;
...
//			printf("ERR_Z %12.8f ",err_z);//printf("ERR_Y %3.7f\tERR_Z %3.7f\t%3.7f\t%3.7f\t",err_y,err_z,(rot_hip[1]+(-50)*(1-leg)+50*leg)*pow(-1,leg+1),com_z);
	...
	prevWalk.set_point[0][2]=(-zreal)*(1-leg)+(-zrreal)*leg;//rot_hip[1];//
	prevWalk.set_point[1][0]=x*leg+xr*(1-leg);
	prevWalk.set_point[1][1]=yreal*leg+yrreal*(1-leg);
	prevWalk.set_point[1][2]=zreal*leg+zrreal*(1-leg);///rot_hip[1];//
...
/*	}*/
	...
	/// saving all data in absolute coods for next walk 
	prevWalk.leg=0;
	prevWalk.set_point[0][0]=390;
	prevWalk.set_point[0][1]=0;
	prevWalk.set_point[0][2]=0;
	prevWalk.set_point[1][0]=390;
	prevWalk.set_point[1][1]=0;
	prevWalk.set_point[1][2]=0;
	
	prevWalk.Omega[0]=0;
	prevWalk.Omega[1]=0;
	prevWalk.Omega[2]=0;
	
	prevWalk.hip_velo[0]=0;
	prevWalk.hip_velo[1]=0;
	prevWalk.hip_velo[2]=0;
