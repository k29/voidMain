...
#define SMOOTSTEPLENGTH(x) ((x) * (x)*(3-2*(x)))
...
#define VMAX 3
#define smoo(x) (6*(x)*(x)-15*(x)+10)*(x)*(x)*(x) 
#define smoo2(x) (3*(x)-2)*(x)*(x)
#define smoo3(x) (1-pow(EULER,VMAX*x))
...
#define COMSTAND sqrt(LLENGTH*LLENGTH-STEPLENGTH*STEPLENGTH-SEPARATION*SEPARATION)-1 //406
// #define COMTRANS sqrt(LLENGTH*LLENGTH-4*STEPLENGTH*STEPLENGTH-(2*SEPARATION+HPLATE)*(2*SEPARATION+HPLATE))
#define COMTRANS 380
#define COMLOW   360
#define LEGHIGH 300
#define ZSWAY 80
...
float com[3]={0},rot_com[3]={0},rot_hip[3]={0};
int offset[33]={0};
...
void check();
void loads();
int generateik(float x,float y,float z);
...
int generate_com()
{
	...
	com[0]=com[0]/(l_wt+r_wt);
	com[1]=com[1]/(l_wt+r_wt);
	com[2]=com[2]/(l_wt+r_wt);
...
int rotate_com(int leg)
{
	...
	imucal();
	roll=-avg_angle[0]*3.14/180;
	pitch=avg_angle[1]*3.14/180;
	yaw=0;//avg_angle[2];
	
	//printf("ROLL %f PITCH %f \t",avg_angle[0],avg_angle[1]);
...
int static_check()
{
	...
	while(exit==0)
	{
		...
		for(i=0;i<2;i++)
		{
			if(abs(avg_angle[i])<=threshold)
			{
				count[i]++;
				change[i]=0;
			}
			else if(abs(avg_angle[i])<=20)
			{
				change[i]=0;
				change[i]=avg_angle[i];
			}	
			else
			{
				count[i]=0;
...
float spline_prediction()
{
	...
	err_out=generatespline(size-1,gx,prev_err);
	for(i=0;i<future_size;i++)
	{
		future_err_z[i]=err_out[(size-2)*4]+err_out[(size-2)*4+1]*(i+1)+err_out[(size-2)*4+2]*pow((i+1),2)+err_out[(size-2)*4+3]*pow((i+1),3);	
		printf("%lf\n",future_err_z[i]);
	}
}
...
///////////// Newton's Forward,Backward,Divided Difference implemented 
/*	-Polynomial of high degree affected by Runge's Phenomenon 
	-Prediction of only single error possible 
	-Shifting to Spline curves for error prediction, while using this system for predicted error corection mechanism for now
*/
...
float pid_z(int leg)
{
	...
	//*for(k=0;k<future_size;k++)	
	{
		...
		for(i=0;i<size;i++)
		{
			...
			future_err_z[k]+=prev_err_z[i][size-i-1]*val/fact(i+1);
			//printf("%lf\t%Le\t%Le\n",prev_err_z[i][size-i-1],val,fact(i));
		}	
	}/
	future_err_z[0]=0;
	int exit=0;
	for(i=0;i<size&&exit==0;i++)
	{
		...
		future_err_z[0]+=prev_err_z[i][0]*val/fact(i);
	}	
	//for(i=0;i<future_size;i++)
	{
		printf("%lf\t",future_err_z[0]);
		future_err_z[0]=0;
	}
	printf("\n");
	//printf("\n*************************COR %lf*********************************************\n",future_err_z[0]);
...
//////////// SIMPLE PID BASED CONTROLLER ////////////////
/* 	- Vibrations encountered are difficult to damp
	- Intergral mechanism causes sudden jerks for weight shift
		- tried integerating correction to reduce above effects. Not much improvement
	- Error correction is much to late for stabilizing. Need for predicting errors for system to work
*/
...
int walk3(int leg,float yin,float yfi,float yrin,float yrfi, float zin, float zfi,float zrin, float zrfi, float thetain, float thetafi,float thetarin, float thetarfi)
{
	...
	float peak_stay=0.75;//0.75
	float peak_end=1;
	float zmax=25+7+7;//*leg;
	float zfreq=1.57;
	float zphase=0;
	...
	for(frame=0;frame<time*fps;frame++,t+=1.0/fps)
	{
		...
		*/
		...
		generateik(x,yreal,zreal+seperation);
		for(i=0;i<5;i++)
		val[i]=mval[i];
		generateik(xr,yrreal,zrreal+seperation);
		for(i=0;i<5;i++)
		valr[i]=mval[i];
...
void initialize_acyut()
{
	...
	tx_packet[2]=0xfe;
	tx_packet[3]=NOM*3+4;
	tx_packet[4]=INST_SYNC_WRITE; //83
	tx_packet[5]=0x1e;
...
int legMoveSpline(float xli, float yli, float zli, float xri, float yri, float zri,
					float xlf, float ylf, float zlf, float xrf, float yrf, float zrf, float ttime,int ulta)
{
	ttime*=FPS;
	float dxl=xlf-xli, dyl=ylf-yli, dzl=zlf-zli, dxr=xrf-xri, dyr=yrf-yri, dzr=zrf-zri;
	float ct;
	for(ct=0; ct<=ttime; ct++)
	{
		go2pos(xli+smoo(ct/ttime)*dxl,yli+smoo(ct/ttime)*dyl,zli+smoo(ct/ttime)*dzl,
				xri+smoo(ct/ttime)*dxr,yri+smoo(ct/ttime)*dyr,zri+smoo(ct/ttime)*dzr,ulta);
		usleep(1000000/FPS);
	}
	return 0;
}

int legMoveSine(float xli, float yli, float zli, float xri, float yri, float zri,
					float xlf, float ylf, float zlf, float xrf, float yrf, float zrf, float ttime,int ulta)
{
	ttime*=FPS;
	float dxl=xlf-xli, dyl=ylf-yli, dzl=zlf-zli, dxr=xrf-xri, dyr=yrf-yri, dzr=zrf-zri;
	float ct;
	#define sine(x) ((sin(((x)-0.5)*3.1416)+1)*0.5)
	for(ct=0; ct<=ttime; ct++)
	{
		go2pos(xli+sine(ct/ttime)*dxl,yli+sine(ct/ttime)*dyl,zli+sine(ct/ttime)*dzl,
				xri+sine(ct/ttime)*dxr,yri+sine(ct/ttime)*dyr,zri+sine(ct/ttime)*dzr,ulta);
		usleep(1000000/FPS);
	}
	#undef sine
	return 0;
...
//Euler
int legMoveExpo(float xli, float yli, float zli, float xri, float yri, float zri,
					float xlf, float ylf, float zlf, float xrf, float yrf, float zrf, float ttime,int ulta)
{
	ttime*=FPS;
	float dxl=xlf-xli, dyl=ylf-yli, dzl=zlf-zli, dxr=xrf-xri, dyr=yrf-yri, dzr=zrf-zri;
	float ct,norm=1/smoo3(1);
	for(ct=0; ct<=ttime; ct++)
	{
		go2pos(xli+norm*smoo3(ct/ttime)*dxl,yli+norm*smoo3(ct/ttime)*dyl,zli+norm*smoo3(ct/ttime)*dzl,
				xri+norm*smoo3(ct/ttime)*dxr,yri+norm*smoo3(ct/ttime)*dyr,zri+norm*smoo3(ct/ttime)*dzr,ulta);
		usleep(1000000/FPS);
	}
	return 0;
}

int legMoveSC(float xli, float yli, float zli, float xri, float yri, float zri,
					float xlf, float ylf, float zlf, float xrf, float yrf, float zrf, float ttime,int ulta)
{
	ttime*=FPS;
	float dxl=xlf-xli, dyl=ylf-yli, dzl=zlf-zli, dxr=xrf-xri, dyr=yrf-yri, dzr=zrf-zri;
	float ct,norm=1/smoo3(1);
	for(ct=0; ct<=ttime*3/4; ct++)
	{
		go2pos(xli+norm*smoo3(ct/ttime)*dxl,yli+norm*smoo3(ct/ttime)*dyl,zli+norm*smoo3(ct/ttime)*dzl,
				xri+norm*smoo3(ct/ttime)*dxr,yri+norm*smoo3(ct/ttime)*dyr,zri+norm*smoo3(ct/ttime)*dzr,ulta);
		usleep(1000000/FPS);
	}
	compliance(ulta,7,39);
	while(ct<=ttime)
	{
		go2pos(xli+norm*smoo3(ct/ttime)*dxl,yli+norm*smoo3(ct/ttime)*dyl,zli+norm*smoo3(ct/ttime)*dzl,
				xri+norm*smoo3(ct/ttime)*dxr,yri+norm*smoo3(ct/ttime)*dyr,zri+norm*smoo3(ct/ttime)*dzr,ulta);
...
int walkahead(int NSleg) //1=right
{
	//Initialize
	// speed(1);
	float tc=1;
	...
	// PAUSE;
	// PAUSE;
	compliance(NSleg,7,32);
	go2pos(COMSTAND,STEPLENGTH,SEPARATION,COMSTAND,-STEPLENGTH,SEPARATION,NSleg);
	//PAUSE;
	// scanf("%c",&c);
	legMoveSpline(COMSTAND,STEPLENGTH,SEPARATION,COMSTAND,-STEPLENGTH,SEPARATION,
					COMTRANS,0,-HPLATE,COMTRANS,-2*STEPLENGTH,SEPARATION*2+HPLATE,0.6*tc,NSleg);
	//PAUSE;
	// scanf("%c",&c);
	legMoveSpline(COMTRANS,0,-HPLATE,COMTRANS,-2*STEPLENGTH,SEPARATION*2+HPLATE,
					COMLOW,0,-HSUP,LEGHIGH,0,ZSWAY,0.4*tc,NSleg);
	//Peak
	// scanf("%c",&c);
	legMoveSpline(COMLOW,0,-HSUP,LEGHIGH,0,ZSWAY,
					COMTRANS,0,-HPLATE,COMTRANS,2*STEPLENGTH,SEPARATION*2+HPLATE,0.4*tc,NSleg);
	// PAUSE;
	// scanf("%c",&c);
	legMoveSine(COMTRANS,0,-HPLATE,COMTRANS,2*STEPLENGTH,SEPARATION*2+HPLATE,
					COMSTAND,-STEPLENGTH,SEPARATION,COMSTAND,STEPLENGTH,SEPARATION,0.8*tc,NSleg);
	//PAUSE;
	// scanf("%c",&c);
	//PAUSE;
	// speed(0);
...
int walkahead2(int NSleg) //1=right
{
	//Initialize
	// speed(1);
	float tc=1.2;
	...
	// PAUSE;
	// PAUSE;
	compliance(NSleg,32,32);
	go2pos(COMTRANS,STEPLENGTH,SEPARATION,COMTRANS,-STEPLENGTH,SEPARATION,NSleg);
	//PAUSE;
	// scanf("%c",&c);
	legMoveSpline(COMTRANS,STEPLENGTH,SEPARATION,COMTRANS,-STEPLENGTH,SEPARATION,
					COMTRANS,0,-HPLATE,COMTRANS,-2*STEPLENGTH,SEPARATION*2+HPLATE,0.8*tc,NSleg);
	//PAUSE;
	compliance(NSleg,10,32);
	// scanf("%c",&c);
	// PAUSE;
	legMoveSpline(COMTRANS,0,-HPLATE,COMTRANS,-2*STEPLENGTH,SEPARATION*2+HPLATE,
					COMLOW,0,-HSUP,LEGHIGH,0,ZSWAY,0.5*tc,NSleg);
	//Peak
	// scanf("%c",&c);
	compliance(NSleg,10,39);
	// PAUSE;
	legMoveSpline(COMLOW,0,-HSUP,LEGHIGH,0,ZSWAY,
					COMTRANS,0,-HPLATE,COMTRANS,2*STEPLENGTH,SEPARATION*2+HPLATE,0.5*tc,NSleg);
	compliance(NSleg,32,32);
	// PAUSE;
	// scanf("%c",&c);
	legMoveSine(COMTRANS,0,-HPLATE,COMTRANS,2*STEPLENGTH,SEPARATION*2+HPLATE,
					COMTRANS,-STEPLENGTH,SEPARATION,COMTRANS,STEPLENGTH,SEPARATION,0.8*tc,NSleg);
	//PAUSE;
	// scanf("%c",&c);
	//PAUSE;
	// speed(0);
...
int walkahead3(int NSleg) //1=right
{
	//Initialize
	// speed(1);
	float tc=1.2;
	...
	//PAUSE;
	// scanf("%c",&c);
	legMoveSmooth(COMTRANS,STEPLENGTH,SEPARATION,COMTRANS,-STEPLENGTH,SEPARATION,
					COMTRANS,0,-HPLATE,COMTRANS,-2*STEPLENGTH,SEPARATION*2+HPLATE,0.8*tc,NSleg);
	//PAUSE;
	compliance(NSleg,10,32);
	// scanf("%c",&c);
	// PAUSE;
	legMoveSmooth(COMTRANS,0,-HPLATE,COMTRANS,-2*STEPLENGTH,SEPARATION*2+HPLATE,
					COMLOW,0,-HSUP,LEGHIGH,0,ZSWAY,0.5*tc,NSleg);
	//Peak
	// scanf("%c",&c);
	compliance(NSleg,10,39);
	// PAUSE;
	legMoveSmooth(COMLOW,0,-HSUP,LEGHIGH,0,ZSWAY,
					COMTRANS,0,-HPLATE,COMTRANS,2*STEPLENGTH,SEPARATION*2+HPLATE,0.5*tc,NSleg);
	compliance(NSleg,32,32);
	// PAUSE;
	// scanf("%c",&c);
...
int walkahead4(int NSleg) //1=right
{
	//Initialize
	// speed(1);
	float tc=1.2;
	...
	compliance(NSleg,39,45);
	// PAUSE;
	legMoveSC(COMLOW,0,-HSUP,LEGHIGH,0,ZSWAY,
					COMTRANS,0,-HPLATE,COMTRANS,2*STEPLENGTH,SEPARATION*2+HPLATE,0.5*tc,NSleg);
	compliance(NSleg,23,27);
	// PAUSE;
	// scanf("%c",&c);
...
int main()
{
	...
	initialize_acyut();
	speed(0);
	// go2pos(COMSTAND,STEPLENGTH,SEPARATION,COMSTAND,-STEPLENGTH,SEPARATION,1);
	go2pos(COMTRANS,STEPLENGTH,SEPARATION,COMTRANS,-STEPLENGTH,SEPARATION,1);
	// PAUSE;
	// speed(1);
	printf("Initialized!\n");
	scanf("%c",&ch);
	...
	// go2pos(300,0,0,300,0,0,0);
	// go2pos(COMSTAND,-STEPLENGTH,SEPARATION,COMSTAND,STEPLENGTH,SEPARATION,0);
	return 0;
}