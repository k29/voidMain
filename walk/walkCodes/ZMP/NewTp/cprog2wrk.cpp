...
//		
		if(veloWtShift<=0)
		{
			response->start[XLIFT].value=false;
...
//		}		
	...
	float HipZ()
	{
		float expHip_z=walk_object->rotHip[1];//+pow(walk_object->hipVelo[1],2)*signum(walk_object->hipVelo[1])/18000;
		prevExpZ=finalZ;
...
int Walk::createComponent(int component)
{
	...
	switch(component)
	{
		...
		case XLAND : 	this->component[XLAND].pointer = new xLand(this,start[XLAND].args[0],start[XLAND].args[1],start[XLAND].args[2],start[XLAND].args[3],start[XLAND].args[4],start[XLAND].args[5],&time);
				...
				break;
		case MOVEY : 	this->component[MOVEY].pointer= new movementY(this,0,time,leg_y,revleg_y,&time);
