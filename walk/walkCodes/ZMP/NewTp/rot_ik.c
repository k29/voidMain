...
float* rotateFromIMU(float *quan,float *result)
{
	...
	convertIKtoIMU(quan,temp);
	printf("IMU F %10.5f  %10.5f  %10.5f\n",temp[0],temp[2],temp[1]);
	revrotateIMU(temp,ang,temp2);
	printf("IMU F %10.5f  %10.5f  %10.5f\n",temp2[0],temp2[2],temp2[1]);
	convertIMUtoIK(temp2,result);
...
void do_stuff()
{
	...
	{
		get_imu_data();
		rotateFromIMU(set_point,result);
		
		printf("A[0] %10.5f A[1] %10.5f\n",current_angle[0],current_angle[1]);
		printf("X : %10.5f Y : %10.5f Z : %10.5f\n",result[0],result[2],result[1]);
		
		fval=generateik(result[0],result[2],result[1],0);
		...
		motor_val[20*leg]-=1*imu_angle[1]*4096/300;
