...
int forward_kinematics(int leg,int motor_val[6])   
{
	...
	frames[0].d=0;
	frames[0].alpha=90;
	frames[0].theta=-90+(1-2*leg)*mval[5];
...
float* generateik(float x, float y, float z,int leg)
{
	...
	ang[0]=phi[0];//+(current_angle[1]*(1-2*leg))*0.2/r2d;
	ang[1]=-theta[1]+phi[1];//-(avg_angle[0]/r2d);
	ang[2]=-theta[2];
	ang[3]=-theta[3]-phi[3];
	ang[4]=phi[5];//+(avg_angle[1]/r2d)*(1-2*leg);;
	//printf("%f\t%f\n",phi[0],ang[0]);
...
int walk4(int leg,float yin,float yfi,float yrin,float yrfi, float zin, float zfi,float zrin, float zrfi, float thetain, float thetafi,float thetarin, float thetarfi)
{
	...
	for(time=0;;time+=1.0/40)
	{
		...
		//change[1]=current_angle[1]*gain_z;//err_z=rot_com[1]*(1-2*leg)-com_z;
		err_y=rot_com[2]-com_y-18;
		err_z=target-rot_com[1]*(1-2*leg);
		printf("ERR %lf\t",err_z);
		...
		for(i=0;i<NOM;i++)
		{
			...
			{
				value=val[0]+offset[tx_packet[7+i*3]]-(1-2*leg)*change[1]*4096/300*7/24;
				if(value>=0 && value<=4096)
				{
					tx_packet[8+i*3]=value%256;
					tx_packet[9+i*3]=value/256;
			...
			{
				value=val[4]+offset[tx_packet[7+i*3]]+(1-2*leg)*change[1]*4096/300*12/24;
				if(value>=0 && value<=4096)
				{
					tx_packet[8+i*3]=value%256;
					tx_packet[9+i*3]=value/256;
			...
			{
				value=valr[0]+offset[tx_packet[7+i*3]]+(1-2*leg)*change[1]*4096/300*7/24;
				if(value>=0 && value<=4096)
				{
					tx_packet[8+i*3]=value%256;
					tx_packet[9+i*3]=value/256;
			...
			{
				value=valr[4]+offset[tx_packet[7+i*3]]-(1-2*leg)*change[1]*4096/300*12/24;
				if(value>=0 && value<=4096)
				{
					tx_packet[8+i*3]=value%256;
					tx_packet[9+i*3]=value/256;
		...
		usleep(16670);
		float eval=fabs(rot_com[1]*(1-2*leg));
		printf(" %f\n",eval);
		if(eval<=2)
		{
			exit++;
...
int go2pos(float xl, float yl, float zl, float xr, float yr, float zr, int ulta)
{
	tx_packet[0]=0xff;
	int adchksum=7+NOM*3; //Hardcoded No of legs
	tx_packet[1]=0xff;
	tx_packet[2]=0xfe;
	tx_packet[3]=NOM*3+4;
	tx_packet[4]=INST_SYNC_WRITE;
	...
	tx_packet[6]=0x02;
	int i_init, mot, revmot, h=0;

	for(i_init=0;i_init<NOM;i_init++)
	{
		tx_packet[7+i_init*3]=bid[i_init];
	}
	float val[5], valr[5], change[5]={0};
	float *fval;
	int value, phi=0, phir=0, leg=ulta;//ulta;
	ulta=0;
	if(leg==0)
	{
		mot=0;
	...
	// if(ulta)
	// {
	// 	generateik(xr,yr,zr,0);
	// 	for(i_init=0;i_init<5;i_init++)	val[i_init]=mval[i_init];
	// 	generateik(xl,yl,zl,0);
	// 	for(i_init=0;i_init<5;i_init++)	valr[i_init]=mval[i_init];
	// }
	// else
	// {
		generateik(xl,yl,zl,0);
	...
	for(i_init=0;i_init<NOM;i_init++)
	{
		if(tx_packet[7+i_init*3]==mot)
		{
			value=val[0]+offset[tx_packet[7+i_init*3]]+(2*leg-1)*change[1]*4096/300*7/24;
			if(value>=0 && value<=4096)
			{
		...
		else if(tx_packet[7+i_init*3]==mot+1)
		{
			value=val[1]+offset[tx_packet[7+i_init*3]]+change[0]*4096/300*3/24;
			if(value>=0 && value<=4096)
			{
				tx_packet[8+i_init*3]=value%256;
				tx_packet[9+i_init*3]=value/256;
		...
		else if(tx_packet[7+i_init*3]==mot+2||tx_packet[7+i_init*3]==mot+12)
		{
		...
		else if(tx_packet[7+i_init*3]==mot+3)
		{
			value=val[3]+offset[tx_packet[7+i_init*3]]-change[0]*4096/300*5/24;
			if(value>=0 && value<=4096)
			{
				tx_packet[8+i_init*3]=value%256;
				tx_packet[9+i_init*3]=value/256;
		...
		else if(tx_packet[7+i_init*3]==mot+4)
		{
			value=rotation(mot+4,phi)+offset[tx_packet[7+i_init*3]];
			if(value>=0 && value<=1024)
			{
				tx_packet[8+i_init*3]=value%256;
				tx_packet[9+i_init*3]=value/256;
		...
		else if(tx_packet[7+i_init*3]==mot+5)
		{
			value=val[4]+offset[tx_packet[7+i_init*3]]+(1-2*leg)*change[1]*4096/300*12/24;
			if(value>=0 && value<=4096)
			{
				tx_packet[8+i_init*3]=value%256;
				tx_packet[9+i_init*3]=value/256;
		...
		else if(tx_packet[7+i_init*3]==revmot)
		{
			value=valr[0]+offset[tx_packet[7+i_init*3]]+(1-2*leg)*change[1]*4096/300*7/24;
			if(value>=0 && value<=4096)
			{
				tx_packet[8+i_init*3]=value%256;
				tx_packet[9+i_init*3]=value/256;
		...
		else if(tx_packet[7+i_init*3]==revmot+1)
		{
			value=valr[1]+offset[tx_packet[7+i_init*3]]+change[0]*4095/300*3/24;
			if(value>=0 && value<=4096)
			{
				tx_packet[8+i_init*3]=value%256;
				tx_packet[9+i_init*3]=value/256;
		...
		else if(tx_packet[7+i_init*3]==revmot+2||tx_packet[7+i_init*3]==revmot+12)
		{
			value=valr[2]+offset[tx_packet[7+i_init*3]];
			if(value>=0 && value<=4096)
			{
				tx_packet[8+i_init*3]=value%256;
				tx_packet[9+i_init*3]=value/256;
		...
		else if(tx_packet[7+i_init*3]==revmot+3)
		{
			value=valr[3]+offset[tx_packet[7+i_init*3]]-change[0]*4096/300*5/24;
			if(value>=0 && value<=4096)
			{
				tx_packet[8+i_init*3]=value%256;
				tx_packet[9+i_init*3]=value/256;
		...
		else if(tx_packet[7+i_init*3]==revmot+4)
		{
			value=rotation(revmot+4,phir)+offset[tx_packet[7+i_init*3]];
			if(value>=0 && value<=1024)
			{
				tx_packet[8+i_init*3]=value%256;
				tx_packet[9+i_init*3]=value/256;
		...
		else if(tx_packet[7+i_init*3]==revmot+5)
		{
			value=valr[4]+offset[tx_packet[7+i_init*3]]-(1-2*leg)*change[1]*4096/300*12/24;
			if(value>=0 && value<=4096)
			{
				tx_packet[8+i_init*3]=value%256;
				tx_packet[9+i_init*3]=value/256;
...
int legMoveSmooth(float xli, float yli, float zli, float xri, float yri, float zri,
					float xlf, float ylf, float zlf, float xrf, float yrf, float zrf, float ttime, int ulta, char c)
{
	ttime*=40; //40
	float dxl=xlf-xli, dyl=ylf-yli, dzl=zlf-zli, dxr=xrf-xri, dyr=yrf-yri, dzr=zrf-zri, norm;
	float ct;
	if(c == 'e') norm=1/expo(1);
	switch(c)
	{
		case 'e':
		for(ct=0; ct<=ttime; ct++)
		{
			go2pos(xli+norm*expo(ct/ttime)*dxl,yli+norm*expo(ct/ttime)*dyl,zli+norm*expo(ct/ttime)*dzl,
				xri+norm*expo(ct/ttime)*dxr,yri+norm*expo(ct/ttime)*dyr,zri+norm*expo(ct/ttime)*dzr,ulta);
			usleep(1000000/40);
		}
		break;
		case 'n':
		for(ct=0; ct<=ttime; ct++)
		{
			go2pos(xlf-norm*expo(ct/ttime)*dxl,ylf-norm*expo(ct/ttime)*dyl,zlf-norm*expo(ct/ttime)*dzl,
				xrf-norm*expo(ct/ttime)*dxr,yrf-norm*expo(ct/ttime)*dyr,zrf-norm*expo(ct/ttime)*dzr,ulta);
			usleep(1000000/40);
		}
		break;
		case 'i':
		for(ct=0; ct<=ttime; ct++)
		{
			norm = ((sin(((ct/ttime)-0.5)*3.1416)+1)*0.5);
			go2pos(xli+norm*dxl,yli+norm*dyl,zli+norm*dzl,
				xri+norm*dxr,yri+norm*dyr,zri+norm*dzr,ulta);
			usleep(1000000/40);
		}
	}
	return 0;
...
int setupWalk()
{
	...
	walk.left.x.lift.lift=20;
	walk.left.x.lift.height=360;
	...
	walk.left.x.load.x_load_dec=-30;//lift/60;
...
// int walk_charli(int leg,float yin,float yfi,float yrin,float yrfi, float zin, float zfi,float zrin, float zrfi, float thetain, float thetafi,float thetarin, float thetarfi)
// {
// 	setupWalk();
// 	stcr=0;
// 	tgive=0;
...
// 	/*/////// X-Lift Control /////
// 	float x_start=walk.left.x.lift.x_start*(1-leg)+walk.right.x.lift.x_start*(leg);
// 	float x_end=walk.left.x.lift.x_end*(1-leg)+walk.right.x.lift.x_end*(leg);
// 	float lift=walk.left.x.lift.lift*(1-leg)+walk.right.x.lift.lift*(leg);
// 	float height=walk.left.x.lift.height*(1-leg)+walk.right.x.lift.height*(leg);
// 	float freq=walk.left.x.lift.freq*(1-leg)+walk.right.x.lift.freq*(leg);
// 	float displacement=walk.left.x.lift.displacement*(1-leg)+walk.left.x.lift.displacement*(leg);	
// 	float phase=walk.left.x.lift.phase*(1-leg)+walk.left.x.lift.phase*(leg);
	
// 	/////// X-Land Control /////
// 	//float dec_time=0.75;	
// 	float x_land_start=walk.left.x.land.x_land_displacement*(1-leg)+walk.right.x.land.x_land_displacement*(leg);//1-x_end    
// 	float x_land_increase_end=walk.left.x.land.x_land_increase_end*(1-leg)+walk.right.x.land.x_land_increase_end*(leg);
// 	float x_land_decrease_start=walk.left.x.land.x_land_decrease_start*(1-leg)+walk.right.x.land.x_land_decrease_start*(leg);
// 	float x_land_end=walk.left.x.land.x_land_end*(1-leg)+walk.right.x.land.x_land_end*(leg);
// 	float x_land_freq=walk.left.x.land.x_land_freq*(1-leg)+walk.right.x.land.x_land_freq*(leg);
// 	float x_land_displacement=walk.left.x.land.x_land_displacement*(1-leg)+walk.right.x.land.x_land_displacement*(leg);
// 	float x_land_phase=walk.left.x.land.x_land_phase*(1-leg)+walk.right.x.land.x_land_phase*(leg);
// 	float x_land_dec=walk.left.x.land.x_land_dec*(1-leg)+walk.right.x.land.x_land_dec*(leg);
	
// 	/////// X-Load Control /////
// 	float x_load_start=walk.left.x.load.x_load_start*(1-leg)+walk.right.x.load.x_load_start*(leg);
// 	float x_load_increase_end=walk.left.x.load.x_load_increase_end*(1-leg)+walk.right.x.load.x_load_increase_end*(leg);
// 	float x_load_decrease_start=walk.left.x.load.x_load_decrease_start*(1-leg)+walk.right.x.load.x_load_decrease_start*(leg);
// 	float x_load_end=walk.left.x.load.x_load_end*(1-leg)+walk.right.x.load.x_load_end*(leg);
// 	float x_load_displacement=walk.left.x.load.x_load_displacement*(1-leg)+walk.right.x.load.x_load_displacement*(leg);//x_land_displacement;
// 	float x_load_phase=walk.left.x.load.x_load_phase*(1-leg)+walk.right.x.load.x_load_phase*(leg);//-1.57;
// 	float x_load_freq=walk.left.x.load.x_load_freq*(1-leg)+walk.right.x.load.x_load_freq*(leg);//3.14;
// 	float x_load_dec=walk.left.x.load.x_load_dec*(1-leg)+walk.right.x.load.x_load_dec*(leg);;//lift/60;
// 	*/
	

// 	/*////// Z-Shift Control /////
// 	float peak_start=walk.left.z_shift.peak_start*(1-leg)+walk.right.z_shift.peak_start*(leg);
// 	float peak_max=walk.left.z_shift.peak_max*(1-leg)+walk.right.z_shift.peak_max*(leg);//0.5
// 	float peak_stay=walk.left.z_shift.peak_stay*(1-leg)+walk.right.z_shift.peak_stay*(leg);//0.75
// 	float peak_end=walk.left.z_shift.peak_end*(1-leg)+walk.right.z_shift.peak_end*(leg);
// 	float zmax=walk.left.z_shift.zmax*(1-leg)+walk.right.z_shift.zmax*(leg);
// 	float zfreq=walk.left.z_shift.zfreq*(1-leg)+walk.right.z_shift.zfreq*(leg);
// 	float zphase=walk.left.z_shift.zphase*(1-leg)+walk.right.z_shift.zphase*(leg);
// 	float zdisplacement=walk.left.z_shift.zdisplacement*(1-leg)+walk.right.z_shift.zdisplacement*(leg);
// 	*/
	

// 	/*////// Y Control /////
// 	float y_start=walk.left.y.y_start*(1-leg)+walk.right.y.y_start*(leg);
// 	float y_end=walk.left.y.y_end*(1-leg)+walk.right.y.y_end*(leg);
// 	float yr_start=walk.left.y.yr_start*(1-leg)+walk.right.y.yr_start*(leg);
// 	float yr_end=walk.left.y.yr_end*(1-leg)+walk.right.y.yr_end*(leg);;
// 	*/
	
// 	/*////// Z Control /////
// 	float z_start=walk.left.z.z_start*(1-leg)+walk.right.z.z_start*(leg);
// 	float z_0=walk.left.z.z_0*(1-leg)+walk.right.z.z_0*(leg);
// 	float z_move=walk.left.z.z_move*(1-leg)+walk.right.z.z_move*(leg);
// 	float z_end=walk.left.z.z_end*(1-leg)+walk.right.z.z_end*(leg);
//         */
//    	 ///////Separation control////
//     	//float seperation=walk.left.misc.seperation*(1-leg)+walk.right.misc.seperation*(leg);
	
// 	//////Turn Control////
// 	//float turn_start=walk.left.turn.turn_start*(1-leg)+walk.right.turn.turn_start*(leg);
// 	//float turn_end=walk.left.turn.turn_end*(1-leg)+walk.right.turn.turn_end*(leg);
...
// 		//change[0]=(rot_com[2]-com_y-18)*gain_y;
// 		//printf("CHANGE [0] %lf\n",change[0]);
// 		//change[1]=(rot_com[1]*pow(-1,leg)-com_z)*pow(-1,leg+1);
// 		//if(abs(change[1])>2)
// 		//change[1]*=gain_z;
// 		//else
// 		//change[1]=0;
...
// 		//getStabilityC(leg);
// 		//setResponse(&zmax,leg);
// 		//rot_com[1]*=pow(-1,leg);
// 		//time=exp_time*(1+tgive);
// 		//t=t_frac*time;
// 		//zfreq=
// 		/*if(ceil(rot_com[1])>=zmax)
// 		{
// 			z_flag++;
// 			rot_com[1]=zmax;
// 		}
// 		if(compare_float(rot_com[1],0)==1&&z_flag==0)
// 		acc_time=0;
// 		else if(z_flag==0)
// 		acc_time=(asin(rot_com[1]*(1+zdisplacement)/zmax-zdisplacement)-zphase)/zfreq*(peak_max-peak_start)+peak_start;
// 		else if(z_flag!=0&&compare_float(rot_com[1],0)==0)
// 		acc_time=(3.14-asin(rot_com[1]*(1+zdisplacement)/zmax-zdisplacement)-zphase)/zfreq*(peak_max-peak_start)+peak_start;
// 		else if(z_flag!=0&&compare_float(rot_com[1],0)==1)
// 		acc_time=1;

// 		if(zmax!=0&&compare_float(acc_time-peak_start,0)==0)
// 		{
// 			if(z_flag==0)
// 			zfreq=(asin(rot_com[1]/zmax*(1+zdisplacement)-zdisplacement)-zphase)/(t_frac-peak_start)*(peak_max-peak_start);
// 			else
// 			zfreq=(3.14-asin(rot_com[1]/zmax*(1+zdisplacement)-zdisplacement)+zphase)/(t_frac-peak_stay)*(peak_end-peak_stay);
			
// 			if(compare_float(zfreq,0)==0&&z_flag==0)
// 			time=tgive*exp_time*((3.14-zphase)/zfreq*(peak_max-peak_start)+peak_start);
// 			else if(compare_float(zfreq,0)==0&&z_flag!=0)
// 			time=tgive*exp_time*((3.14-zphase)/zfreq*(peak_end-peak_stay)+peak_stay);
// 		}
// 		*/
// 	//	getStabilityC(leg);
// 	//	time=setTime(stcr);
// 	//	t=t_frac*time;
// 	/*		printf("Frame %d ROTCOM %f COM %f ERR %f TIME %f ACC TIME : %lf Z_FLAG %d\n",frame,rot_com[1],com_z,err_z,t_frac,acc_time,z_flag);*/
// 			printf("%f X %f XR %f ZSHIFT %f Y %f YR %f \n",t_frac,x,xr,z_shift,yreal,yrreal);
// 	/*		printf("FREQ %f TIME %f\t",zshift.zfreq,time);*/
// 	/*		printf("SC %f TGIVE %f\n",stcr,tgive);*/
// 	/*		//err_z+=err_z>0?-3:3;*/
...
int walk5(int leg,float yin,float yfi,float yrin,float yrfi, float zin, float zfi,float zrin, float zrfi, float thetain, float thetafi,float thetarin, float thetarfi)
{
	...
	float add_z=0;
	float add_x=0;
	float newz, newx, zset, xset, tset;
	int step_down=0;
	float hipendcoods[3];
	...
	for(frame=0;walk_end==0;frame++)
	{
		...
		zcom[0]=rot_hip[1]*(2*leg-1);
		ycom[0]=rot_hip[2];
		
		momn_z=calcMomentum(zcom,10);
		momn_y=calcMomentum(ycom,10);
		
		phase_z=identifyPhaseZ(((rot_hip[1]+(-50)*(1-leg)+50*leg)*(2*leg-1)));
		...
		{
			printf("\n\n*********Sequence 2!!!!\n\n\n\n");
			err_z=com_z-((rot_hip[1]+(-50)*(1-leg)+50*leg)*(2*leg-1));
			err_y=com_y-rot_hip[2];
			//printf("ERR_Z %12.8f ",err_z);//printf("ERR_Y %3.7f\tERR_Z %3.7f\t%3.7f\t%3.7f\t",err_y,err_z,(rot_hip[1]+(-50)*(1-leg)+50*leg)*(2*leg-1),com_z);
			...
			//feet_orient[1]=0.8*current_angle[1]*(1-2*leg);
			seq_crit=(rot_hip[1]*(1-2*leg)+pow(momn_z,2)*signum(momn_z));
			...
			*/
			...
			if(phase_z==2 || !firsttime)
			{
				...
				if(touchdown>5)
				{
					...
					float SINUS = fabs(sin(current_angle[1]*3.14/180));
					float COSY = fabs(cos(current_angle[1]*3.14/180));
					
					newx=xset - min( (xr*(SINUS*SINUS + (1-COSY)*COSY)) , 100*pow(10,3*(tset-t_frac) ) );
					newz=zset+(2*leg-1)*1.1*xset*fabs(SINUS*COSY - (1-COSY)*SINUS);
					// add_x=0.6*add_z;
					printf("new_z %f\tnew_x %f\n",newz,newx);
				}
				if(touchdown<=3 && set_land==0)
				{
					// exit(0);
					// sequence=4;
			...
			land_angle[0]=1.1*current_angle[0]*1023/180;
			land_angle[1]=1.2*current_angle[1]*1023/180;

			feet_orient[0]=1.1*current_angle[0]*1023/180;
			feet_orient[1]=1.2*current_angle[1]*1023/180;
		}
		else if(sequence==2)
		{
			printf("\n\n*********Sequence 2!!!!\n\n\n\n");
		...
		// {
		// 	// exit(0);
		// 	printf("\n\nEntering Recovery Module! / Seq 4\n\n\n\n");
		// 	// use_imu=0;
		// 	//Call Setup Walk
		// 	//Walk Init
		// 	legMoveSmooth(xr,yr,zr,x,y,z,
		// 					xr,0,10,xr,0,10,1,leg,'i');
		// 	printf("1\n");
		// 	legMoveSmooth(xr,0,10,xr,0,10,
		// 					390,0,-10,350,0,20,0.7,leg,'e');
		// 	printf("2\n");	
		// 	legMoveSmooth(390,0,-10,350,0,20,
		// 					390,yrfi,zrfi,390,yfi,zfi,1,leg,'n');
		// 	printf("3\n");
		// 	// walk5(leg,y,yfi,yr,yrfi,z,zfi,zr,zrfi,0,0,0,0);
		// 	imu_initialize();
		// 	// usleep(100);
		// 	return 1;
		// }
		...
		motor_val[20*leg]+=feet_orient[0];
		motor_val[20*(1-leg)]-=feet_orient[1];
...
int walk2(int leg,float yin,float yfi,float yrin,float yrfi, float zin, float zfi,float zrin, float zrfi, float thetain, float thetafi,float thetarin, float thetarfi, float time)
{
	...
	for(;t<time;t+=(float)1/60)
	{
		...
		*/
		...
		zr+=z_shift*(2*leg-1)*sin(change[1]*3.14/180);
		z-=z_shift*(2*leg-1)*sin(change[1]*3.14/180);
		...
		*/
		...
		for(i=0;i<NOM;i++)
		{
			...
			if(tx_packet[7+i*3]==mot+4)
			{
				value=tx_packet[8+i*3]+tx_packet[9+i*3]*256;
				tx_packet[adchksum]+=tx_packet[8+i*3]+tx_packet[9+i*3];
				//value-=floor((1-2*leg)*ch*1023/300);
				value=rotation(mot+4,phi)+offset[tx_packet[7+i*3]];
