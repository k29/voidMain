...
#define CRITERROR 10
#define RANDERROR 50
#define OVERSHOOT 1.2
#define RECTIFRAC 0.85
...
int go2pos(float xl, float yl, float zl, float xr, float yr, float zr, int leg)
{
	tx_packet[0]=0xff;
	...
	// Correction System:
	float dev = abs(xl-xlm)+abs(yl-ylm)+abs(zl-zlm);
	if(dev<RANDERROR)
	{
		if(dev>CRITERROR)
		{
	...
	printf("\n\nCOM wala: X: %f Y: %f Z: %f\n",rot_com[0],rot_com[1],rot_com[2]);
	printf("Hip wala: X: %f Y: %f Z: %f\n\n\n",rot_hip[0],rot_hip[1],rot_hip[2]);
