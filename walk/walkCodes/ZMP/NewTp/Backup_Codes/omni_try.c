...
float* setWalkX(float t_frac, walk_x x, int sequence)
{
...
float setWalkShift(float t_frac,walk_zshift zshift, int phase)
{
	...
	zshift_val=zshift.zmax*(sin(zshift.zfreq*(t_frac-zshift.peak_stay)/(zshift.peak_end-zshift.peak_stay)+zshift.zphase+1.57)+zshift.zdisplacement)/(1+zshift.zdisplacement);
...
int identifySequence(float hip_z)
{
	int phase=-1;
	if(hip_z<5&&hip_z>-5)
	phase=0;
...
int walk5(int leg,float yin,float yfi,float yrin,float yrfi, float zin, float zfi,float zrin, float zrfi, float thetain, float thetafi,float thetarin, float thetarfi)
{
	...
	for(frame=0;sequence<3;frame++)
	{
		...
		phase=identifySequence(((rot_hip[1]+(-50)*(1-leg)+50*leg)*pow(-1,leg+1)));
		
		
		retval=setWalkX(t_frac,x_control,sequence);
		x=retval[0];
		xr=retval[1];//+add_xr;
		...
		if(sequence==0)
		{
			///// Pre Lift COM trajectory control 
			// Getting COM to lift pos
...
//			printf("ERR_Z %12.8f ",err_z);//printf("ERR_Y %3.7f\tERR_Z %3.7f\t%3.7f\t%3.7f\t",err_y,err_z,(rot_hip[1]+(-50)*(1-leg)+50*leg)*pow(-1,leg+1),com_z);
			...
			//if(err_z>10||err_z<-5)
			cor_zr[sequence]+=0.1*err_z;
			cor_z[sequence]-=0.1*err_z;
			if(err_y>5||err_y<-5)
			cor_y+=0.5*err_y;
			...
			if(inRange(err_z,-5,10)==1&&(phase==0||phase==1))//com_z<15//&&inRange(err_y,-5,5)==1)
			sequence_count++;
			else
			sequence_count=0;
			
			if(sequence_count==2||(imu_init==0&&t_frac>x_control.lift.x_start))//||(compare_float(t_frac,lift.x_start)))
			//if(t_frac>0.2)
			{
		...
		else if(sequence==1)
		{
			mag_xr=0;
			//if(lift==0)
			err_z=-(rot_hip[1]+(-50)*(1-leg)+50*leg)*pow(-1,leg+1);
			//else
			//err_z=com_z-(rot_hip[1]+(-50)*(1-leg)+50*leg)*pow(-1,leg+1);
			err_y=rot_hip[2];
			...
			*/
			...
			if(lift==1&&touchdown<=5)
			touchdown_count++;
			else
			touchdown_count=0;
			
			if((touchdown_count>=5&&lift==1)||(imu_init==0&&t_frac>x_control.lift.x_end))
			{
				//printf("************ SEQ ************\n");	
				seq_change=1;
				//sequence++;
...
//			printf("ERR_Z %12.7f\t",err_z);
			if(inRange(err_z,-5,7)==0)
			{
				cor_z[sequence]=0;
				cor_zr[sequence]+=0.4*(err_z);
				//if(lift==0)
			...
			land_angle[1]=0.8*current_angle[1]*pow(-1,leg);
			feet_orient[0]=0.8*current_angle[0];
			feet_orient[1]=0;
		...
		else if(sequence==2)
		{
			...
			err_z=com_z-((rot_hip[1]+(-50)*(1-leg)+50*leg)*pow(-1,leg+1));
			cor_zr[sequence]=0.3*err_z+scurve(cor_zr[sequence-1],0,seq2_count,5);
			cor_z[sequence]=0.3*err_z+scurve(cor_z[sequence-1],0,seq2_count++,5);
			add_xr=0;
			//err_z=0;
			...
			if(sequence_count==2||(imu_init==0&&t_frac>1.2))//||(compare_float(t_frac,lift.x_start)))
			{
				seq_change=1;
				//sequence++;
				x_control.lift.x_end=t_frac+(x_control.lift.x_end-x_control.lift.x_start);
	...
	//	printf("SigCZR %2d ",signum(-diff_shift*cor_zr[sequence]));
		...
		cor_zr[sequence]+=0.27*(momn_z-diff_shift);
		
		
		if(abs(cor_zr[sequence])<50&&abs(cor_z[sequence])<50)
		{
		...
		//printf("X %10.5f XR %10.5f Y %10.5f YR %10.5f Z %10.5f ZR %10.5f \n",x,xr,yreal,yrreal,zreal,zrreal);
		...
		for(walk_i=0,target_i=0;walk_i<6;walk_i++)
		{
			if(walk_i==4)
			{
				motor_val[20*(1-leg)+walk_i]=rotation(20*(1-leg)+walk_i,phi)+offset[20*(1-leg)+walk_i];
				continue;
			}
			else if(walk_i==2)
...
int walk3(int leg,float yin,float yfi,float yrin,float yrfi, float zin, float zfi,float zrin, float zrfi, float thetain, float thetafi,float thetarin, float thetarfi)
{
	...
	for(frame=0;sequence<3;frame++)  //frame<time*fps
	{
		gettimeofday(&tv,NULL); /*gets the current system time into variable tv */ 
		walk_prev_frame = (tv.tv_sec *1e+6) + tv.tv_usec;
		
		/*if(compare_float(rot_hip[1]-prev_hip,0)==1)
		{
			if(compare_float(com_z-prev_com_z,0)==1)
			change_ratio=1;
			else
			printf("********\n");
		}
		else
		change_ratio=(com_z-prev_com_z)/(rot_hip[1]-prev_hip);
		*/
		//printf("\n\n\nRATIO %f\n",change_ratio);
		if(pauseSeq==0)
		{
			//if(change_ratio>0&&change_ratio<2)
			//t+=change_ratio/50;
			//else
			t+=1.0/fps;
		}
		...
		if(sequence>=1)
		{
			...
			xr=lift.height+setWalkLoad(t_frac,load)+add_xr;
		}
	...
	//	printf("Zreal %f Zrreal %f ",zreal,zrreal);	
		printf("X %10.5f XR %10.5f Y %10.5f YR %10.5f Z %10.5f ZR %10.5f PHI %10.5f PHIR %10.5f ZREAL %10.5f ZRREAL %10.5f \n",x,xr,y,yr,z,zr,phi,phir,zreal,zrreal);
...
/*		printf("FRAME %2d\tTOUCHDOWN %2d\t",frame,touchdown);*/
/*		//printf("Rleg %7f\t",rot_rleg[0]);*/
/*		printf(" ANGLE 0 %f ANGLE 1 %f\t",current_angle[0],current_angle[1]);  */
/*		//printf("HEIGHT%3f\t",390-x);*/
/*		printf("COR_Z %8f\tCOR_ZR %8f\t",cor_z,cor_zr);*/
/*		printf("SEQ %2d\n",sequence);*/
		//printf("ERR Y %f \n",err_y);

/*		if(touchdown==0&&correcting==0)*/
/*		{*/
/*			if(err_z<15&&err_z>-5)*/
/*			{*/
/*				//cor_z=err_z*0;*/
/*			}*/
/*			else if(err_z>=15)*/
/*				cor_z+=(err_z-10)*z_shift/(2*zshift.zmax);*/
/*			else if(err_z<=-5)*/
/*				cor_z+=(err_z)*z_shift/(2*zshift.zmax);*/
/*			*/
/*			cor_z-=0.5*(cor_z-prev_cor_z);*/
/*			prev_cor_z=cor_z;*/
/*			ret_time=t_frac;*/
/*		*/
/*		}*/  
/*		else*/
/*		{*/
/*			cor_z=scurve(prev_cor_z,0,t_frac-ret_time,1-ret_time);*/
/*		}*/
		/// Feasibility proof of new z balance system 
			
		/*if(touchdown==0)
		{
			if(correcting==0&&(err_z>15||err_z<-3))
			{
				add_xr=0;
				count=0;
				correcting=1;
			}
			else if(correcting==1)
			{
				add_xr=5*sin(3.14*count/20);
				cor_z=10*sin(3.14*count/20);
				count++;
				if(count==15)
				correcting=0;
			}
...
//			printf("ERR_Z %12.8f ",err_z);//printf("ERR_Y %3.7f\tERR_Z %3.7f\t%3.7f\t%3.7f\t",err_y,err_z,(rot_hip[1]+(-50)*(1-leg)+50*leg)*pow(-1,leg+1),com_z);
			...
			if(inRange(err_z,-5,10)==1&&com_z<15)//&&inRange(err_y,-5,5)==1)
			sequence_count++;
			else
			sequence_count=0;
			
			if(sequence_count==2)//||(compare_float(t_frac,lift.x_start)))
			{
				sequence++;
				lift.x_end=t_frac+(lift.x_end-lift.x_start);
				lift.x_start=t_frac;
				touchdown_count=0;
			}
...
//			printf("ERR_Z %12.7f\t",err_z);
		...
		else if(sequence==2)
		{
			...
			if(inRange(err_z,-5,10)==1&&rot_hip[0])//&&inRange(err_y,-5,5)==1)
			sequence_count++;
			else
			sequence_count=0;
		...
		//printf("Zshift %f\t",z_shift);
		/*if(touchdown==0)
		{
			if(err_z>14||err_z<-5)
			{
				cor_z=0.2*(err_z);//+0.1*(0.2*err_z-cor_z);
				add_xr=0.5*(390-x-rot_rleg[0]);//+0.2*(0.5*(390-x-rot_rleg[0])-add_xr);
			}
			else
			{
				add_xr=0;
		...
		///// Feet Correction v1.0		
		if(touchdown==1&&feet_corr==1)
		{
			feet_orient[0]=scurve(land_angle[0],0,t_frac-land_time,1-land_time)*4096/300;
			feet_orient[1]=scurve(land_angle[1],0,t_frac-land_time,1-land_time)*4096/300;
		}
		else
		{
			land_angle[0]=0.8*current_angle[0];
		...
		///printf("feet_orient[0] %f feet_orient[1] %f\n",feet_orient[0],feet_orient[1]); 
		*/
		//feet_orient[0]=0;
		//feet_orient[1]=0;
		for(i=0;i<NOM;i++)
		{
			if(tx_packet[7+i*3]==mot)
			{
				value=val[0]+offset[tx_packet[7+i*3]]+feet_orient[1];//-pow(-1,leg)*change[1]*4096/300*7/24;
				if(value>=0 && value<=4096)
				{
					tx_packet[8+i*3]=value%256;
					tx_packet[9+i*3]=value/256;
			...
			{
				value=val[1]+offset[tx_packet[7+i*3]]+feet_orient[0];//+change[0]*4096/300*3/24;
				if(value>=0 && value<=4096)
				{
					tx_packet[8+i*3]=value%256;
					tx_packet[9+i*3]=value/256;
			...
			{
				value=valr[0]+offset[tx_packet[7+i*3]]-feet_orient[1];//+pow(-1,leg)*change[1]*4096/300*7/24;
				if(value>=0 && value<=4096)
				{
					tx_packet[8+i*3]=value%256;
					tx_packet[9+i*3]=value/256;
			...
			{
				value=valr[1]+offset[tx_packet[7+i*3]];//+feet_orient[0];//+change[0]*4095/300*3/24;
				if(value>=0 && value<=4096)
				{
					tx_packet[8+i*3]=value%256;
					tx_packet[9+i*3]=value/256;
		...
		gettimeofday(&tv,NULL); /*gets the current system time into variable tv */ 
		walk_curr_frame = (tv.tv_sec *1e+6) + tv.tv_usec;
		//printf("%ld\n",walk_curr_frame-walk_prev_frame);
		if(walk_curr_frame-walk_prev_frame<20000)
		usleep(20000-(walk_curr_frame-walk_prev_frame));
...
{	
	...
	float zmax=45+0*(leg)+zfi/6-zin/6;///+2*(1-leg);
	...
	for(;t<time;t+=(float)1/60)
	{
		...
		*/
		...
		fval=generateik(390,yrreal,zrreal,1-leg);
		for(walk_i=0,target_i=0;walk_i<6;walk_i++)
		{
			if(walk_i==4)
			{
...
int main()
{
	...
	{
		//printf("%d\n",read_position(5));
		walk2(flag,0,30,0,-30,0,20,0,20,0,0,0,0,1);
		walk2(1-flag,-30,0,30,0,20,0,20,0,0,0,0,0,1);
		//flag=1-flag;
