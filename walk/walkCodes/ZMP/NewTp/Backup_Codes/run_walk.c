...
int make_walk(float y_in,float x_in,float dir_in,float y_fi,float x_fi)
{
	x_in*=-1;
	x_fi*=-1;
	int flag=0;
	float prev_step=0;
	float distance=sqrt(pow(x_in-x_fi,2)+pow(y_in-y_fi,2));
	float theta=atan2(x_fi-x_in,y_fi-y_in)*180/3.14-dir_in;
	
	printf("DIS %f THETA %F\n",distance,theta);
	float dist=distance;
	float phi=theta;
	if(phi>=0&&phi<=180)
	flag=1;
	...
	while(phi>0)
	{
		...
		{
			printf("TURN\n");
			walk3(flag,0,0,0,0,0,0,0,0,0,10,0,10);
			walk3(1-flag,0,0,0,0,0,0,0,0,10,0,10,0);
			phi-=20;	
		}
		else
		{	
			printf("TURN\n");
			walk3(flag,0,0,0,0,0,0,0,0,0,phi,0,phi);
			walk3(1-flag,0,0,0,0,0,0,0,0,phi,0,phi,0);
			phi-=phi;
		}
	}
	while(dist>0)
	{
		...
		{
			printf("WALK\n");
			walk3(flag,-prev_step,20,prev_step,-20,0,0,0,0,0,0,0,0);
			flag=1-flag;	
			dist-=(20+prev_step);
			prev_step=20;
		}
		...
		{	
			printf("WALK\n");
			walk3(flag,-prev_step,dist,prev_step,-dist,0,0,0,0,0,0,0,0);
			flag=1-flag;
			prev_step=dist;
			dist-=dist;
	...
	walk3(flag,-prev_step,0,prev_step,0,0,0,0,0,0,0,0,0);
...
int walk3(int leg,float yin,float yfi,float yrin,float yrfi,float zin,float zfi,float zrin,float zrfi,float thetain,float thetafi,float thetarin,float thetarfi)
{
	...
	for(frame=0;frame<time*fps;frame++,t+=1.0/fps)
	{
		...
		printf("X %f XR %f Y %f YR %f PHI %f PHIR %f\n",x,xr,y,yr,phi,phir); 
		for(i=0;i<NOM;i++)
		{
			if(tx_packet[7+i*3]==mot)
			{
