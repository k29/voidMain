			...
			*/
			...
			if(phase_z==2)
			{
				...
				if(touchdown>=10)
				{
					...
					add_start=t_frac;
					printf(" t_frac %f\n\n\n\n\n\n\n\n",t_frac);
					add_max=1;//add_start+0.4;//0.4;//.247;
					}
					if(t_frac<add_max&&t_frac>=add_start)
					{
					add_z=targetz-targetz*(cos(addzfreq*(t_frac-add_start)/(add_max-add_start)));
					add_x=(targetx-xset2)*(sin(addzfreq*(t_frac-add_start)/(add_max-add_start)));
						printf("add_z %f\tadd_x %f\trot_hipendcoods%f\tX: %f\n",add_z,add_x,rot_hipendcoods[0],x);
					}
					else printf("Haggu\n");
				}
				
				// add_x=a*sq((t_frac-addx_start)/(addx_max-addx_start))+b*((t_frac-addx_start)/(addx_max-addx_start))+c;
				// add_x=-1.5*abs( x * ( 1 - fabs( cos(current_angle[1]*3.14/180) ) ) );
