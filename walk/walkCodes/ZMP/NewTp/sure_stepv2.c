...
int setupWalk()
{
	...
	walk.left.z_shift.zmax=55;
	walk.left.z_shift.zfreq=1.57;//3.14;
	walk.left.z_shift.zphase=0;//-1.57;
	walk.left.z_shift.zdisplacement=0;//1;
...
float setWalkShift(float t_frac,walk_zshift zshift, int phase)
{
	...
	{
		zshift_val=zshift.zmax*(sin(zshift.zfreq*(t_frac-zshift.peak_stay)/(zshift.peak_end-zshift.peak_stay)+zshift.zphase+zshift.zfreq)+zshift.zdisplacement)/(1+zshift.zdisplacement)+10*(sin(3.14*((t_frac-zshift.peak_stay)/0.1)-1.57)+1)/2;
	}
...
inline int walk5(int leg,float yin,float yfi,float yrin,float yrfi, float zin, float zfi,float zrin, float zrfi, float thetain, float thetafi,float thetarin, float thetarfi)
{
	...
	for(frame=0;walk_end==0;frame++)
	{
		...
		for (com_i = 0; com_i < 3; com_i += 1)
		{
			hip_velo[com_i]=(rot_hip[com_i]*pow(-1,leg+1)-prev_rot_hip[com_i])/(1000)*fps;
			prev_rot_hip[com_i]=rot_hip[com_i]*pow(-1,leg+1);
			rad[com_i]=rot_hip[com_i]/1000*pow(-1,leg+1);
			
		}
		
		rad[1]+=(-50/1000)*(1-leg)+50/1000*leg;
...
//			printf("ERR_Z %12.8f ",err_z);//printf("ERR_Y %3.7f\tERR_Z %3.7f\t%3.7f\t%3.7f\t",err_y,err_z,(rot_hip[1]+(-50)*(1-leg)+50*leg)*pow(-1,leg+1),com_z);
			...
			//if(t_frac>0.2)
			{
				...
				if(use_imu==1)
				{
					...
					zshift.peak_end=x_control.lift.x_end+(zshift.peak_end-zshift.peak_stay);
					zshift.peak_stay=x_control.lift.x_end;
		...
		else if(sequence==1)
		{
			...
			*/
			...
			if((touchdown_count>=5&&lift==1&&(step_down==0||x_control.land.x_land_end<t_frac))||(x_control.lift.x_end<t_frac&&use_imu==0))
			{
				...
				x_control.land.x_land_increase_end=t_frac+(zshift.peak_end-zshift.peak_stay)/2;
				x_control.land.x_land_decrease_start=t_frac+(zshift.peak_end-zshift.peak_stay)/2;
				x_control.land.x_land_end=t_frac+(zshift.peak_end-zshift.peak_stay);
	...
	//	printf("SigCZR %2d ",signum(-diff_shift*cor_zr[sequence]));
		...
		cor_zr[sequence]+=0.99*(-hip_velo[1]*1000-diff_shift);
	...
	walk.prevWalk.set_point[0][0]=x;
	walk.prevWalk.set_point[0][1]=yreal;
	walk.prevWalk.set_point[0][2]=zreal;
	walk.prevWalk.set_point[1][0]=xr;
	walk.prevWalk.set_point[1][1]=yrreal;
	walk.prevWalk.set_point[1][2]=zrreal;
	
	walk.prevWalk.Omega[0]=Omega[0];
	walk.prevWalk.Omega[1]=Omega[1];
	walk.prevWalk.Omega[2]=Omega[2];
	
	walk.prevWalk.hip_velo[0]=hip_velo[0];
	walk.prevWalk.hip_velo[1]=hip_velo[1];
	walk.prevWalk.hip_velo[2]=hip_velo[2];
	
	walk.prevWalk.hip_coods[0]=rot_hip[0];
	walk.prevWalk.hip_coods[1]=rot_hip[1];
	walk.prevWalk.hip_coods[2]=rot_hip[2];
	
	walk.prevWalk.cor_z=cor_z[sequence];
	walk.prevWalk.cor_zr=cor_zr[sequence];
	
	gettimeofday(&walk.prevWalk.endTime,NULL);
