...
#define serialusb2d "A900fDpz"//"A900fDpz"//"A900fDhp" //"A600cBa7" //"A600cURy"  //"A900fC5U"//nd USB2D: A7003N1d  A7003NDp	 A7005LC8 //A700eST2
...
float setWalkShift(float t_frac,walk_zshift zshift, int phase)
{
	...
	{
		zshift_val=0;
		printf("dshjdlakjkldasjkldsjakdfjlcmdsnfk.jfengfskjsdkfvlkdshf\n");
	}
	return zshift_val;
}
...
int walk5(int leg,float yin,float yfi,float yrin,float yrfi, float zin, float zfi,float zrin, float zrfi, float thetain, float thetafi,float thetarin, float thetarfi)
{
	...
	// float c=350;
	float addx_max=0.33;
	float addx_start=0.083;
	float add_max=0.083;
	float add_start=0.33;
	float addzfreq=1.57;
	float add_t=0.1;
...
//			printf("ERR_Z %12.8f ",err_z);//printf("ERR_Y %3.7f\tERR_Z %3.7f\t%3.7f\t%3.7f\t",err_y,err_z,(rot_hip[1]+(-50)*(1-leg)+50*leg)*pow(-1,leg+1),com_z);
			...
			if(phase_z==2)
			{
				...
				if(touchdown>=5)
				{
					if(nxset)
					{
						...
						add_start=t_frac;
						// printf(" t_frac %f\n\n\n\n\n\n\n\n",t_frac);
						//add_mid=add_start+0.1;//add_start+0.4;//0.4;//.247;
						add_t=0.07;
						printf("t_frac %f\n\nadd_start+add_t %f\n\n",t_frac,add_start+add_t);
						// zshift.peak_end=max(add_start+add_t,zshift.peak_end);
					
						}
					else if(t_frac < add_start+add_t)// && t_frac > addx_start)
					{
						printf("here");
						newz=(targetz-30)*((t_frac-add_start)/add_t)+30;
						// newx=scurve2(xset,targetx,t_frac-add_start,add_mid-add_start);
						// newx=scurve2(targetx-xset,targetx2,t_frac-add_mid,add_max-add_mid);//(targetx-xset)*(t_frac-add_start)/(0.4);
						newx=(targetx-xset)*(t_frac-add_start)/add_t+xset;
						printf("Setting z %f\tSetting x %f\trot_hipendcoods%f\tX: %f\n",newz,newx,rot_hipendcoods[0],x);
					}
					else if(t_frac > add_start+add_t)
					{
						printf("overshot\n\n\n");
						set_land=0;
					}
				...
				// newx=a*sq((t_frac-addx_start)/(addx_max-addx_start))+b*((t_frac-addx_start)/(addx_max-addx_start))+c;
				// newx=-1.5*abs( x * ( 1 - fabs( cos(current_angle[1]*3.14/180) ) ) );
				
				if(touchdown<5&&set_land==0)   //&&lift==1&&
				{
					printf("Yo man!\n");
					exit (0);
					set_land=1;
					x_control.land.x_land_start=t_frac-0.2;
		...
		else if(sequence==2)
		{
			...
			//newz=scurve(save,0,seq2_count,10);
	...
	//	printf("SigCZR %2d ",signum(-diff_shift*cor_zr[sequence]));
		...
		//printf("ADDZ %10.5f \t ADDX %10.5f \n",newz,newx);
		// z+=newz;
		// x+=newx;
		if(!nxset && step_down && !set_land)
		{
			z=newz;
			x=newx;
		}
		...
		printf("\n\nx: %f \t y: %f \t z: %f\n\n\n",x,yreal,zreal);
		fval=generateik(x,yreal,zreal+10,leg);
		...
		fval=generateik(xr,yrreal,zrreal+10,1-leg);
...
int main()
{
	...
	// {
		//printf("%d\n",read_position(5));
		walk5(flag,-STEP,STEP,STEP,-STEP,ZR_END,0,Z_END,0,0,0,0,0);
		// sleep(3);//flag=1-flag;
