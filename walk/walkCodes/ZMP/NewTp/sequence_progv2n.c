...
int walk5(int leg,float yin,float yfi,float yrin,float yrfi, float zin, float zfi,float zrin, float zrfi, float thetain, float thetafi,float thetarin, float thetarfi)
{
	...
	for(frame=0;walk_end==0;frame++)
	{
		...
		momn_z=0;//calcMomentum(zcom,10);
		momn_y=0;//calcMomentum(ycom,10);
		...
		retval=setWalkX(t_frac,x_control,sequence,step_down&firsttime);
		x=retval[0];
		xr=retval[1];//+add_xr;
		...
		else if(sequence==10)
		{
			...
			int sep =sqrt((xr-x)*(xr-x)+(zrreal+zreal)*(zrreal+zreal))/2;
			int xd = sqrt(390*390-sep*sep);
			printf("Separation is %d\n",sep);
			legMoveSmooth(xr,yrreal,zrreal,x,yreal,zreal,
							xd,0,-sep,xd,0,sep,1.4,leg,'i');
			printf("1\n");
			legMoveSmooth(xd,0,-sep,xd,0,sep,
							390,0,-20,390-sep,0,30,0.7,leg,'i');
			printf("2\n");
			legMoveSmooth(390,0,-20,390-sep,0,30,
							390,yrfi,zrfi,390,yfi,zfi,1,leg,'i');
			printf("3\n");
