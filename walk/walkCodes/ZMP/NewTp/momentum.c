...
int walk3(int leg,float yin,float yfi,float yrin,float yrfi, float zin, float zfi,float zrin, float zrfi, float thetain, float thetafi,float thetarin, float thetarfi)
{
	...
	err_z=(rot_hip[1]+(-50)*(1-leg)+50*leg)*pow(-1,leg+1);int z_flag=0;
	float acc_time;
...
/*		//err_z+=err_z>0?-3:3;*/
		...
		printf("FRAME %d TOUCHDOWN %d HIP COOD %f ",frame,touchdown,(rot_hip[1]+(-50)*(1-leg)+50*leg)*pow(-1,leg+1));
...
/*		if(touchdown==1)*/
/*		{*/
/*			cor_y=0.8*scurve(err_y,0,*/
/*		}*/
/*		*/
		if(touchdown==1&&feet_corr==1)
		{
			// IT BEGINS HERE -- ENTHU
