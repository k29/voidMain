...
float rot_hip[3]={0},rot_rleg[3],hip[3]={0};
/// BASE VALUES 
...
//	float peaktime=0.75;
	...
	for(frame=0;frame<time*fps;frame++,t+=1.0/fps)
	{
		...
		err=(-1+2*leg)*(4096.00/300)*part*(-current_angle[1]);
		errtrail[pos++%5]+=
		// err=0;
		// current_angle[1]+=err;
				////TURN////
		...
		*/
		...
		for(i=0;i<NOM;i++)
		{
			...
			{
				value=val[4]+offset[tx_packet[7+i*3]]-err;
			...
			{
				value=valr[4]+offset[tx_packet[7+i*3]]+err;
				printf("value: %d\n",value);
				printf("error: %f\n",err);
