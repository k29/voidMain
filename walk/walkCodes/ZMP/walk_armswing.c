...
#define serialusb2d "A900fDpA" //"A600cBa7" //"A600cURy"  //"A900fC5U"//nd USB2D: A7003N1d  A7003NDp	 A7005LC8	  A700eST2
...
float com[3]={0},rot_com[3]={0},prev_rot_com[3]={0};
int offset[33]={0};
...
{	
	...
	*/
	...
	prev_rot_com[0]=rot_com[0];
	prev_rot_com[1]=rot_com[2];
	prev_rot_com[2]=rot_com[2];
	rot_com[0]=-rot_z;
...
float pid_z(float err)
{
	...
	int i=0;
	float sum_err_z=0;//sum_err(prev_err_z,1);
...
inline int walk3(int leg,float yin,float yfi,float yrin,float yrfi, float zin, float zfi,float zrin, float zrfi, float thetain, float thetafi,float thetarin, float thetarfi)
{
	...
	float x_load_increase_end=0.05;
	float x_load_decrease_start=0.05;
	float x_load_end=0.1;
...
//	float peaktime=0.75;
	...
	float com_y,com_z,exp_com_z;
	float com_change_real=1,com_change_exp=1;
	float prev_cor_z=1;
	...
	for(frame=0;frame<time*fps;frame++,t+=1.0/fps)
	{
		...
		z_shift=scurve2(0,zmax,t_frac-peak_start,peak_max-peak_start);
		else if(t_frac>peak_stay&&t_frac<peak_end)
		z_shift=scurve2(zmax,0,t_frac-peak_stay,peak_end-peak_stay);
		...
		if(t_frac<z_start)
		{
			z=zin;
			zr=zrin;
		}scurve(zin,zin+zrin,t_frac-z_start,z_0-z_start);
		...
		if(((t+1.0/fps)/time)<z_com_start)
		exp_com_z=0;
		else if(((t+1.0/fps)/time)>z_com_start&&((t+1.0/fps)/time)<z_com_peak)
		exp_com_z=z_com_max*sin(1.57*(t_frac-z_com_start)/(z_com_peak-z_com_start));
		else if(((t+1.0/fps)/time)>=z_com_peak&&((t+1.0/fps)/time)<z_com_stay)
		exp_com_z=z_com_max;
		else if(((t+1.0/fps)/time)>=z_com_stay&&((t+1.0/fps)/time)<z_com_end)
		exp_com_z=z_com_max*sin(1.57*(t_frac-z_com_stay)/(z_com_end-z_com_stay)+1.57);
		else
		exp_com_z=0;
		...
		*/
		...
		zreal+=z_shift;
		zrreal-=z_shift;
		fval=generateik(x,yreal,zreal+seperation,leg);
		for(i=0;i<5;i++)
		val[i]=fval[i];
		fval=generateik(xr,yrreal,zrreal+seperation,1-leg);
		for(i=0;i<5;i++)
		valr[i]=fval[i];
		...
		com_change_real=(rot_com[1]-prev_rot_com[1])*pow(-1,leg);
		com_change_exp=exp_com_z-com_z;
		prev_cor_z=cor_z;
		cor_z=pid_z(err_z-com_change_exp);
		
		cor_z=(prev_cor_z/com_change_real)*cor_z;
