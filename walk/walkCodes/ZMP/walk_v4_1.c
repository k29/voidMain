...
{	
	...
	*/
	float rot_x=cos(pitch)*x+sn(pitch)*z;
...
float* generateik(float x, float y, float z,int leg)
{
	...
	ang[4]=phi[5];
	//printf("%f\n",phi[5]);
	mval[0]=(ang[0]*r2d*4096/300)+cons[0]+z*1.1;//-current_angle[1]*4096/360*pow(-1,leg)+z*1.1;
...
int walk3(int leg,float yin,float yfi,float yrin,float yrfi, float zin, float zfi,float zrin, float zrfi, float thetain, float thetafi,float thetarin, float thetarfi)
{
	...
	for(frame=0;frame<time*fps;frame++,t+=1.0/fps)
	{
		...
		*/
		...
		{
			exp_velo_z=z_com_max*3.14/(z_com_peak-z_com_start)*cos(3.14*(future_frac-z_com_start)/(z_com_peak-z_com_start)-1.57)/2;
			//com_z=z_com_max*(sin(3.14*(t_frac-z_com_start)/(z_com_peak-z_com_start)-1.57)+1)/2;//0*(1-(t_frac-z_com_start)/(z_com_peak-z_com_start))+z_com_max*((t_frac-z_com_start)/(z_com_peak-z_com_start));
		}
		else if(t_frac>z_com_peak&&t_frac<=z_com_stay)
		{
		...
		{
			exp_velo_z=-z_com_max*3.14/(z_com_end-z_com_stay)*cos(3.14*(future_frac-z_com_stay)/(z_com_end-z_com_stay)-1.57)/2;
			//com_z=z_com_max-z_com_max*(sin(3.14*(t_frac-z_com_stay)/(z_com_end-z_com_stay)-1.57)+1)/2;
		}
		else
		{
		...
		*/
		...
		if(future_frac<=z_com_start)
		future_com_z=0;
		else if(future_frac>z_com_start&&future_frac<=z_com_peak)
		future_com_z=z_com_max*(sin(3.14*(future_frac-z_com_start)/(z_com_peak-z_com_start)-1.57)+1)/2;//0*(1-(future_frac-z_com_start)/(z_com_peak-z_com_start))+z_com_max*((future_frac-z_com_start)/(z_com_peak-z_com_start));
		else if(future_frac>z_com_peak&&future_frac<=z_com_stay)
		future_com_z=z_com_max;//+z_static_inc*(sin(2*3.14*(future_frac-z_com_peak)/(z_com_stay-z_com_peak)-1.57)+1)/2;
		else if(future_frac>=z_com_stay&&future_frac<=z_com_end)
		future_com_z=z_com_max-z_com_max*(sin(3.14*(future_frac-z_com_stay)/(z_com_end-z_com_stay)-1.57)+1)/2;
		else
		future_com_z=0;
		*/
		//exp_velo_z=(com_z-past_com_z)*fps;
		//exp_acc_z=((future_com_z-com_z)*fps-exp_velo_z)*fps;
	
		//printf("EXP COM PAST : %f PRESENT : %f FUTURE : %f VELO : %f ACC : %f \n",past_com_z,com_z,future_com_z,exp_velo_z,exp_acc_z);		
		/*if(frame!=0)
		...
		*/
		...
		z_shift=zmax*(sin(zfreq*(t_frac-peak_stay)/(peak_end-peak_stay)+zphase+1.57)+zdisplacement)/(1+zdisplacement);
		...
		z_shift=zmax*(exp(0.693*(t_frac-peak_start)/(peak_max-peak_start))-1);
		else if(t_frac>peak_stay&&t_frac<=peak_end)
		z_shift=zmax*(exp(0.693*(1-(t_frac-peak_stay)/(peak_end-peak_stay)))-1);
		else if(t_frac>peak_end)
		z_shift=0;
		//z_shift=com_z;
		*/
		...
		com_y=yrin-(yrin-yrfi)*(t_frac-y_com_start)/(y_com_end-y_com_start);
		else
		com_y=yrfi;
