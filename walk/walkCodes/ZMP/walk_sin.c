...
float com[3]={0},rot_com[3]={0},rot_hip[3]={0},rot_rleg[3];
int offset[33]={0};
...
int walk3(int leg,float yin,float yfi,float yrin,float yrfi, float zin, float zfi,float zrin, float zrfi, float thetain, float thetafi,float thetarin, float thetarfi)
{
	...
	for(frame=0;frame<time*fps;frame++,t+=1.0/fps)
	{
		...
		*/
		...
		{
			if(z_flag==0)
			zfreq=(asin(rot_com[1]/zmax*(1+zdisplacement)-zdisplacement)-zphase)/(acc_time-peak_start)*(peak_stay-peak_start);
			else
			zfreq=(3.14-asin(rot_com[1]/zmax*(1+zdisplacement)-zdisplacement)-zphase)/(acc_time-peak_start)*(peak_stay-peak_start);
			
			time=exp_time*((3.14-zphase)/zfreq*(peak_max-peak_start)+peak_start+peak_end-peak_stay);
			printf("FREQ %f TIME %f\n",zfreq,time);
		}
