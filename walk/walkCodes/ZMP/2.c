...
float pid_z(float z_shift,float zmax)
{
	float pk=0.;//1
	float pi=0.0;
	float pd=-1;//-0.5
	...
	int i=0;
	float sum_err_z=sum_err(prev_err_z,1);

	cor_z=(pk*err_z+pi*sum_err_z+pd*(err_z-prev_err_z[0]))*(sin(2*3.14*z_shift/zmax -1.57)+1)/2;
	if(abs(cor_z)>50)
	{
		printf("*******************************************************************\n");
		cor_z=50*cor_z/abs(cor_z);

	}
	for(i=119;i>0;i--)
	prev_err_z[i]=prev_err_z[i-1];
	
	prev_err_z[0]=err_z;

	
	printf("SUM : %lf\tCORR : %lf\n",sum_err_z,cor_z);
...
inline int walk3(int leg,float yin,float yfi,float yrin,float yrfi, float zin, float zfi,float zrin, float zrfi, float thetain, float thetafi,float thetarin, float thetarfi)
{
	...
	float peak_stay_start=0.6;//0.75
...
//	float peaktime=0.75;
	...
	for(frame=0;frame<time*fps;frame++,t+=1.0/fps)
	{
		...
		{ z_shift=-zmax*(cosh(k1*(t_frac-peak_start)/(peak_end-peak_start))-2);
		...
		{	seperationr=sep_max*(sin(sep_freq*(t_frac-sep_start)/(sep_trans-sep_start)))+seperation;
...
void initialize_acyut()
{
	...
	float *fval;
	int value;
	fval=generateik(390,-30,32+seperation,0);
	for(h=0;h<5;h++)
	val[h]=fval[h];
	fval=generateik(390,0,-32,0);
	for(h=0;h<5;h++)
	valr[h]=fval[h];
