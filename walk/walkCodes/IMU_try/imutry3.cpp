...
int main()
{
	...
	for(int i=0;i<t*fps;i++)
	{
		...
		fval=generateik(x,y+yLeft,z+zLeft,0);
		for(h=0;h<5;h++)
		val[h]=fval[h];
		...
		target_left[2]=val[2];
		target_left[3]=val[3];
		target_left[4]=rotation(4,0);
		target_left[5]=val[4];

		//printf("%lf \n",val[4]);
		fval=generateik(xr,yr+yRight,zr+zRight,0);
		for(h=0;h<5;h++)
		val[h]=fval[h];
		...
		target_right[0]=val[0];
		target_right[1]=val[1];
		target_right[2]=val[2];
		target_right[3]=val[3];
		target_right[4]=rotation(24,0);
		target_right[5]=val[4];
		
		forward_kinematics(0,target_left);
		forward_kinematics(1,target_right);
		...
		zRight = 0.1*(rot_com[2]-z);
		zLeft	= 0.1*(rot_com[2]-z);
		
		yRight =0.1*rot_com[1];
		yLeft  =0.1*rot_com[1];
	...
	while(x>340)
	{
		...
		usleep(50000);
		printf("\n%lf\n",imu.pitch-initial_pitch);
		while(abs(imu.pitch-initial_pitch)>1.50)
		{
			printf("\n%lf\n",imu.pitch-initial_pitch);
			if((imu.pitch-initial_pitch)>=0)
			{
				z=z-5;
				zr=-z;
			}
