...
enum val{D,THETA};
float newval[2];
float gr=50,gtheta=20;
float* read_walk_data()
{
	newval[D]=gr;
	newval[THETA]=gtheta;
	return newval;
...
void call_walk()
{
	...
	float r,theta,theta1;
	float *val; 
	float *state= endState;
	val = globalWalkR;
	int flag=0;
	
	printf("Call_walk working\n");
	state[YR]=lYin;
	printf("Call_walk working\n");	state[Y]=slYin;
	printf("Call_walk working\n");	state[YV]=vYin;
	printf("Call_walk working\n");	state[ZR]=lZin;
	...
	while(theta1>15)
	{
		if(theta>0)
		{
		state = dribble(flag,state[YR],state[Y],state[YV],vTTar,state[ZR],state[Z],-state[ZV],state[ZV],0,15,0,0);
		state = dribble(1-flag,state[YR],state[Y],state[YV],vTTar,state[ZR],state[Z],-state[ZV],state[ZV],state[RR],0,state[R],0);
		}
		else if(theta<0)
		{
		state = dribble(flag,state[YR],state[Y],state[YV],vTTar,state[ZR],state[Z],-state[ZV],state[ZV],0,0,0,15);
		state = dribble(1-flag,state[YR],state[Y],state[YV],vTTar,state[ZR],state[Z],-state[ZV],state[ZV],state[RR],0,state[R],0);
		}
		theta1=theta1-15;
	}
	...
	{
	state = dribble(flag,state[YR],state[Y],state[YV],vTTar,state[ZR],state[Z],-state[ZV],state[ZV],0,0,0,theta1);
	state = dribble(1-flag,state[YR],state[Y],state[YV],vTTar,state[ZR],state[Z],-state[ZV],state[ZV],state[RR],0,state[R],0);
	}
	while(x<r1)
	{
