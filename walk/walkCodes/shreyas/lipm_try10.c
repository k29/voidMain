...
*/	
enum SupportPhase{DSP1,DSP2,SSP};
float lYin  = -5;
float slYin = 5;
float vYin  = 50;
float vYfi  = 50;
...
float* turn1(int leg, float legZin, float supLegZin, float veloZin, float veloZfi, float theta,float zMax)
{
	float m;
	int fps = 60;
	int sleep = 1000000.0/(float)fps;
	...
	float D_dsp1Time = 0.07;
	float D_dsp2Time = 0.3;
	...
	for(walkTime = 0.0/fps; walkTime<=stepTime; walkTime +=timeInc)
	{
		...
		printf("X\t%lf\tXR\t%lf\tZ\t%lf\tZR\t%lf\t",x,xr,z,zr);
		///printf("X\t%lf\tXR\t%lf\tZ\t%lf\tZR\t%lf\tY\t%lf\tYR\t%lf\n",x,xr,z,zr,y,yr);
		fval=generateik(x,y,z+feetSeperation,leg);
			...
			if(walk_i==4)
		 	{
				if(state==SSP)
				{
					m=(walkTime-dsp1Time)/sspTime;
					motor_val[20*leg+walk_i]=rotation(20*leg+walk_i,theta*m*m*(3-2*m))+offset[20*leg+walk_i];
					
				}
				else if(state==DSP2)
					motor_val[20*leg+walk_i]=rotation(20*leg+walk_i,theta)+offset[20*leg+walk_i];
				else
					motor_val[20*leg+walk_i]=rotation(20*leg+walk_i,0)+offset[20*leg+walk_i];
				printf("%d\n",motor_val[20*leg+walk_i]);
				continue;
			}
			else if(walk_i==2)
			motor_val[20*leg+10+walk_i]=fval[target_i]+offset[20*leg+10+walk_i];
...
/*	leg=1-leg;
	...
	veloZfi=140;
	sspZAmp   = zMax;
	sspZPhs   = asinh(veloZin*Tc/sspZAmp);
	sspZTime  = Tc * (asinh(veloZfi*Tc/sspZAmp) - sspZPhs);
	sspZSupin = sspZAmp * cosh(sspZPhs);
	sspZSupfi = sspZAmp * cosh(sspZTime/Tc + sspZPhs);
	
	sspTime   = sspZTime;
	stepTime  = sspTime + dsp1Time + dsp2Time;
	startX = dsp1Time;
	stopX  = stepTime-dsp2Time;
	sspZin    = legZin + (-veloZin)*dsp1Time;
	P_sspZAmp = zMax;
	P_sspZPhs = asinh(-veloZfi*Tc/P_sspZAmp);
	P_sspZin  = P_sspZAmp * cosh(P_sspZPhs);
	P_legZin  = P_sspZin - (-veloZfi)* D_dsp2Time;
	P_veloZfi = veloZfi + (veloZfi + veloZin)/2;
	P_sspZTime= Tc * (asinh(P_veloZfi*Tc/P_sspZAmp) - P_sspZPhs);
	sspZfi    = P_legZin + veloZfi*D_dsp1Time; 
	
	legZfi    = sspZfi - veloZfi*dsp2Time;
	supLegZfi = sspZSupfi + veloZfi*dsp2Time;
	for(walkTime = 0.0/fps; walkTime<=stepTime; walkTime +=timeInc)
	{
		if(walkTime < dsp1Time)
		{
		...
		for(walk_i=0,target_i=0;walk_i<6;walk_i++)
		{
			...
			{
				if(state==DSP1)	
					motor_val[20*(1-leg)+walk_i]=rotation(20*(1-leg)+walk_i,theta)+offset[20*(1-leg)+walk_i];		
				else if(state==SSP)
				{
					m=(walkTime-dsp1Time)/sspTime;
					motor_val[20*(1-leg)+walk_i]=rotation(20*(1-leg)+walk_i,theta*(1-m*m*(3-2*m)))+offset[20*(1-leg)+walk_i];
				}
				else
					motor_val[20*(1-leg)+walk_i]=rotation(20*(1-leg)+walk_i,0)+offset[20*(1-leg)+walk_i];
				printf("%d\n",motor_val[20*(1-leg)+walk_i]);
				continue;
			}
			else if(walk_i==2)
			motor_val[20*(1-leg)+10+walk_i]=fval[target_i]+offset[20*(1-leg)+10+walk_i];
...
float* turn2(int leg, float legZin, float supLegZin, float veloZin, float veloZfi, float theta,float zMax)
{
	float m;
	int fps = 60;
	int sleep = 1000000.0/(float)fps;
	...
	float D_dsp1Time = 0.3;
	float D_dsp2Time = 0.07;
...
void rotate_leg(int leg, int theta)
{
	int h=0;
	float val[5];
	float *fval;
	...
	if(leg==4)
	{ 
		for(walkTime=0;walkTime<t*fps;walkTime++)
		{
			m=(float)walkTime/(float)(t*fps);
			...
			if(m<0.25)
				zr=(int)(zmin+(zmax-zmin)*(2-4*m)*(4*m));
			else if(m>0.75)
				zr=(int)(zmin+(zmax-zmin)*(2-4*(m-0.5))*(4*(m-0.5)));
			else
				zr=zmax;
			z=-zr;
			...
			motor_val[2]=val[2]+offset[2];
			motor_val[3]=val[3]+offset[3];
			motor_val[4]=rotation(4,theta*(m*m*(3-2*m)))+offset[4];
			motor_val[5]=val[4]+offset[5];
			motor_val[6]=512+offset[6];
			motor_val[7]=698+offset[7];
			motor_val[8]=512+offset[8];
			...
			usleep(sleep);
			printf("l1: x:%lf y:%lf z:%lf l2: x:%lf y:%lf z:%lf theta:%d\n",x,y,z,xr,yr,zr,motor_val[4]);
			if(generatePacket()==1 && iwrite(MOTOR_DATA,&fd,&ftdic1,tx_packet,tx_packet[3]+4)>0);
		}
	zmax=10;
		for(walkTime=0;walkTime<t*fps;walkTime++)
		{
			...
			motor_val[2]=val[2]+offset[2];
			motor_val[3]=val[3]+offset[3];
			motor_val[4]=rotation(4,theta-theta*(m*m*(3-2*m)))+offset[4];
			motor_val[5]=val[4]+offset[5];
			motor_val[6]=512+offset[6];
			motor_val[7]=698+offset[7];
			motor_val[8]=512+offset[8];
	...
	{ 
		for(walkTime=0;walkTime<t*fps;walkTime++)
		{
			...
			motor_val[22]=val[2]+offset[22];
			motor_val[23]=val[3]+offset[23];
			motor_val[24]=rotation(24,theta*(m*m*(3-2*m)))+offset[24];
			motor_val[25]=val[4]+offset[25];
			motor_val[26]=512+offset[26];
			motor_val[27]=325+offset[27];
			motor_val[28]=512+offset[28];
	...
	zmax=10;
		for(walkTime=0;walkTime<t*fps;walkTime++)
		{
			...
			motor_val[22]=val[2]+offset[22];
			motor_val[23]=val[3]+offset[23];
			motor_val[24]=rotation(24,theta-theta*(m*m*(3-2*m)))+offset[24];
			motor_val[25]=val[4]+offset[25];
			motor_val[26]=512+offset[26];
			motor_val[27]=325+offset[27];
			motor_val[28]=512+offset[28];
...
//float veloZin = -120;
//float veloZfi = 120;
/*void dribble(int leg, float iflegYin, float supLegYin, float veloYin, float veloYfi, float legZin, float supLegZin)
{
	int fps = 60;
	int sleep = 1000000.0/(float)fps;
	float timeInc =1.0/(float)fps;
...
int main()
{
	...
	int flag=0;
	float *state;
/*	state=turn1(flag,10,10,-160,160,0,35);
	state=turn2(1-flag,state[ZR],state[Z],-160,160,0,35);
	while(!kbhit2())
	{
		state=turn1(flag,state[ZR],state[Z],-160,160,0,35);	
		state=turn2(1-flag,state[ZR],state[Z],-160,160,0,35);
		
	}	
//	rotate_leg(4,15);	*/
//	compliance(0,10,10);
	state = dribble(flag,lYin,slYin,vYin,vYfi,lZin,slZin,vZin,vZfi);
