...
void dribble(int leg, float legYin, float supLegYin, float veloYin, float veloYfi, float legZin, float supLegZin)
{
	...
	float Tc = sqrt(476.0/9810.0);  // Pendulum Constant : Ratio of height of COM plane from Ground plane to acc due to gravity
 	float Ty = -0.17;               // Y Time Constant : Ratio of COM position form origin (pendulum attachment to ground) along Y axis to velocity of COM in Y direction  
	float Kz = 1;
...
//	while(!kbhit2())
	{
		dribble(flag, -30, 30, 50,50 , 6, 6);
	}	
	while(!kbhit2())
	{
		flag=1-flag;
		dribble(flag,YYRR,YY, 150,150 ,ZZRR, ZZ);
//		while(!kbhit2());
	...
	compliance(0,32,32);
//		flag=1-flag;
//		dribble(flag,YYRR,YY, 150,140, , 6);	
	#if PLOT==1
	f=fopen("WALK","w");
	if(f!=NULL)
	{
