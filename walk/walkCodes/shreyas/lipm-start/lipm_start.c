...
float* dribble2(int leg, float legYin, float supLegYin, float veloYin, float veloYfi, float legZin, float supLegZin, float veloZin, float veloZfi)
{
	zMax=20;
	...
	for(walkTime = 0.0/fps; walkTime<=stepTime; walkTime +=timeInc)
	{
		...
		else if (walkTime >= dsp1Time && walkTime <= dsp1Time + sspTime)
		{
			x  = height - lift * (sin(xfreq*((walkTime-startX)/(stopX-startX))+xPhase) + displacement)/(1+displacement);
			xr = height;
			y  = 0;//linear(-sspYin,-sspYfi, walkTime-dsp1Time,sspTime);
		...
		else if (walkTime > dsp1Time+sspTime )//&& walkTime<=stepTime
		{
			...
			x  = height;
			xr = height;
			y  = 0;//-sspYfi - veloYfi_d*(walkTime-dsp1Time-sspTime);
			yr = 0;//-sspYSupfi - veloYfi_d*(walkTime-dsp1Time-sspTime);
...
int main()
{
	...
	int h=0;
	compliance(0,10,10);
	state = dribble2(flag,lYin,slYin,vYin,vYfi,lZin,slZin,vZin,vZfi);
	
	//lipm_generatorSSP(flag, 0, 0.010, -0.01 ,0.75);
	flag=1-flag;
	while(h<4)
	{
		state = dribble2(flag,state[YR],state[Y],state[YV],vTTar,state[ZR],state[Z],-state[ZV],state[ZV]);
		flag=1-flag;
		h++;
	}
	state = dribble(flag,lYin,slYin,state[YV],vTTar,state[ZR],state[Z],-state[ZV],state[ZV]);
	flag=1-flag;
	while(!kbhit2())
	{
