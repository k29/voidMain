...
#define MINenAngle 20.0 //minimum angle to encircle
...
//walk thread-walk to r,theta------>this function will keep on reading *Flag_moving* and *flag_encircle*, if true then it will use the updated r and theta or other data, make acYut walk or encircle and reset the flag.
		...
		while(1)//main Behaviour starts
		{
			...
			if(globalflags.LocalizationState==0)
			{
				if(conf<TH)
				{
					...
					nolocframecount++;
					if(nolocframecount>200)//if framecount exceeds 200 then turn a little and search for more land marks
					{
			...
			if(GOALKEEPER==0)
			{
				...
				{
					printf("[behaviour] Generate Path flag set\n");
					globalflags.p=GENERATE;
				}

				printf("Flag moving main local = %d\n",Flag_moving_mainlocal);
				if(Flag_moving_mainlocal==0)//not moving
				{
					...
					if(globalflags.p==GENERATE)
					{
						...
						switch(pathreturnval)
						{
							case DOWALK:
										cvWaitKey();
										printf("[behaviour]-------Path Returned DOWALK--------\n");
										moveAcYut(pathobj.next.r,pathobj.next.theta,0,WALK);//REMEMBER Pathobj.next.theta has the opposite convention  hence the MINUS sign is put in path code!!
							...
							case DOENCIRCLE:
										...
										printf("[behaviour]-------Path Returned DOENCIRCLE--------\n");
										cvWaitKey();
										moveAcYut(0,0,pathobj.NextEncircleAngle,ENCIRCLE);
										//cvWaitKey();
										break;
							case DOKICK:
										printf("[behaviour]-------Path Returned DOKICK--------\n");
										...
										break;
							case NOPATH:
										printf("[behaviour]-------Path Returned NOPATH--------\n");
										if(abs(pathobj.next.r)<STEPLEN || abs(pathobj.next.theta)<MINTHETA)
										{
											continue;
										}
										else
										{
											moveAcYut(pathobj.next.r,pathobj.next.theta,0,WALK);
										}
										if(pathobj.next.r==0 && pathobj.next.theta==0)
