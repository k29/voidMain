...
public:
    ...
    SegmentImages(IplImage* &seg_yellow, IplImage* &seg_blue, IplImage* &seg_red, CamCapture &cam) :
        my_seg_yellow(seg_yellow), my_seg_blue(seg_blue), my_seg_red(seg_red), my_cam(&cam)
    {}
};
...
void FeatureDetection::getBlobs(CamCapture &cam)
{
	parallel_for( blocked_range2d<size_t>(0, IMAGE_WIDTH, 16, 0, IMAGE_HEIGHT, 32),     
                  SegmentImages(seg_yellow,seg_blue,seg_red,cam) );
...
//	cvSmooth(histogram, histogram, CV_GAUSSIAN, 3);
	...
	for(int x = 0; x < IMAGE_WIDTH/4; x++)
	{
		...
		// returnPixel1C(seg_black, x, returnPixel1C(histogram, x, 0)) = 127;
	}
 	...
 	for(std::vector<Obstacle>::iterator it = o.begin(); it != o.end();)
 	{
        if(!obstacleReturn)
 		...
 		else
 		{
 			...
 			if(mm.updated==1)
 			{
 				//Add code here for updating obstacles
                printf("Obstacle before updating:\n");
                printf("Obstacle Distance: %f Angle: %f Counter: %d\n",it->distance, it->angle, it->counter);
 				it->angle -= mm.theta;

 				double d = mm.r/10.0;
 				double r = it->distance;
 				double t = deg2rad(it->angle);
 				double l;
 				l = sqrt(d*d + r*r - 2.0*d*r*cos(t));
                printf("l = %lf\n", l);
 				it->distance = l;
 				if(l <=0)
 				{
 				...
 				{
 					it->angle = asin((r*sin(t))/l)*180.0/CV_PI;
 					it->angle -= mm.theta2;
 				}

                printf("Obstacle after updating:\n");
                printf("Obstacle Distance: %f Angle: %f Counter: %d\n",it->distance, it->angle, it->counter);
