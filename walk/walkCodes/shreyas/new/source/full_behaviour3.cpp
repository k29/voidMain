...
void moveAcYut(double r,double theta,double enc_angle,WalkCommands com)//This Function sends data to walk thread using mutexlocks
{
	...
	if(abs(prevwalkstr.theta)>abs(theta))//considering movement towards ball
	{
		//good enough u r moving close to 
	}
	...
	if(localflagmoving==0)
	{
		...
		{
			pthread_mutex_lock(&mutex_walkstr);
			//set if any deviation global_CHANGEWALKFLAG=1;
			walkstr.r=r*10;
			walkstr.theta=theta;
...
int main()
{
	...
	#endif
	{
		...
		MovingState movingState = MOVE_TO_BALL;
			...
			if(GOALKEEPER==0)
			{
				...
				#endif
				{
					...
					if(globalflags.p==GENERATE)
					{
						...
						#define DISTANCE_FROM_BALL 50.0 	// in cm
