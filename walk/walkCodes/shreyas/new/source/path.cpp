...
//^^^^^^^^^^^^^^^^^^^^^^^^^^^new encircling and kicking and positioning^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
	...
	for(int i=0,j=0;i<len_curve-1;i++)
	{
		if(!tree.is_onCircle(curve[i],curve[i+1]))
		{
			...
			curvenext[j].theta=(atan2((tree[curve[i+1]].y-tree[curve[i]].y),(tree[curve[i+1]].x-tree[curve[i]].x)));		
			j++;
...
void Path::updatePathPacket(PathPacket &pathpackvar)
{
	if(path_completed_flag)
	{
		...
		if(k<29)
		{
			pathpackvar.finalpath[k+1].x=-1;
			pathpackvar.finalpath[k+1].y=-1;
		}	
	}
}