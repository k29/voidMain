...
{
	int fps = 60;
	int sleep = 16670;//000000.0/(float)fps;
	float timeInc =1.0/(float)fps;
	
	float feetSeperation = 0;
	float hipLength = 100;
	
	float Tc = sqrt(476.0/9810.0);  // Pendulum Constant : Ratio of height of COM plane from Ground plane to acc due to gravity
...
//	float sspSupLegYin = supLegYin - veloYin*dsp2Time;
	...
	int sspMotVal = 1703;
	int sspRMotVal = 1703;
	for(walkTime = 0.0; walkTime<=stepTime+1.0/fps; walkTime +=timeInc)
	{
		...
		else if (walkTime >= dsp1Time && walkTime < dsp1Time + sspTime)
		{
			x  = height - lift * (sin(xfreq*((walkTime-startX)/(stopX-startX))+xPhase) + displacement)/(1+displacement);
			xr = height;
			y  = scurve(sspLegYin,sspLegYfi, walkTime-dsp1Time,sspTime);
		...
		else if (walkTime >= dsp1Time+sspTime )//&& walkTime<=stepTime
		{
			...
			x  = height;
			xr = height;
			y  = sspLegYfi - veloYin*(walkTime-dsp1Time-sspTime);
			yr = sspSupLegYfi - veloYfi*(walkTime-dsp1Time-sspTime);
			z  = sspLegZfi - veloZfi*(walkTime-dsp1Time-sspTime);
			zr = sspSupLegZfi + veloZfi*(walkTime-dsp1Time-sspTime);
			printf("DSP2\t");
		}
		else 
		{
...
//	while(!kbhit2())
	{
		dribble(flag,5,5 ,0.1, 0.1,ZZRR,ZZ);
//		usleep(16670*2);	
		flag=1-flag;
