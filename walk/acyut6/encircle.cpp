...
float* turn(float legYin, float supLegYin, float veloYin, float veloYfi, float legZin, float supLegZin, float veloZin, float veloZfi, float zMax,float legRotin, float legRotfi, float supLegRotin, float supLegRotfi, Leg *leg, Leg *revLeg, Communication *comm)
{
...
float* dribble(float legYin, float supLegYin, float veloYin, float veloYfi, float legZin, float supLegZin, float veloZin, float veloZfi, float zMax,float legRotin, float legRotfi, float supLegRotin, float supLegRotfi, Leg *leg, Leg *revLeg, Communication *comm)
{
	printf("VYin\t%lf\tVyfi\t%lf\n",veloYin,veloYfi);
	int fps = 60;
	int sleep = 1000000.0/(float)fps;
...
//	imu.init();
	...
	float *state;
	state = endState;
	state = dribble(lYin,slYin,vYin,vYfi,lZin,slZin,vZin,vZfi,75,0,0,0,0,bot.right_leg,bot.left_leg,&comm);
	while(!quit)
	{
		state = turn(state[YR],state[Y],state[YV],vTTar,state[ZR],state[Z],-state[ZV],state[ZV],85,state[RR],0,state[R],0,bot.left_leg,bot.right_leg,&comm);
		state = turn(state[YR],state[Y],state[YV],vTTar,state[ZR],state[Z],-state[ZV],state[ZV],75,0,0,0,0,bot.right_leg,bot.left_leg,&comm);
	}
/*	while(!quit)
	{
		...
		state = dribble(state[YR],state[Y],state[YV],vTTar,state[ZR],state[Z],-state[ZV],state[ZV],75,0,0,0,0,bot.right_leg,bot.left_leg,&comm,&imu);
	}
	state = dribble(state[YR],state[Y],state[YV],vTTar,state[ZR],state[Z],-state[ZV],state[ZV],75,0,0,0,0,bot.left_leg,bot.right_leg,&comm,&imu);
	i=0;
	while(i<12)
	{
		i++;
		state = turn(state[YR],state[Y],state[YV],vTTar,state[ZR],state[Z],-state[ZV],state[ZV],75,state[RR],0,state[R],0,bot.right_leg,bot.left_leg,&comm,&imu);
		state = turn(state[YR],state[Y],state[YV],vTTar,state[ZR],state[Z],-state[ZV],state[ZV],75,0,10,0,0,bot.left_leg,bot.right_leg,&comm,&imu);
	}*/
		return 0;
}
