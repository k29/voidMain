...
int main()
{
	...
	float *state;
	state = endState;
	state = dribble(lYin,slYin,vYin,vYfi,lZin,slZin,vZin,vZfi,75,0,0,0,0,bot.left_leg,bot.right_leg,&comm,&imu);
	while(1)
	{
		state = dribble(state[YR],state[Y],state[YV],vTTar,state[ZR],state[Z],-state[ZV],state[ZV],75,state[RR],0,state[R],0,bot.right_leg,bot.left_leg,&comm,&imu);
