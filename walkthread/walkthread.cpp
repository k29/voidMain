...
int main()
{
	...
	footstepmain(walk.velocity2() , foot1[j].delta_y , j%2 , pathnumber , foot1 , i);
	j=0;
	int check = 1;
	while(j<i-1 && check == 1)
	...
	if (check == 0)
		...
		footstepmain(walk.velocity2() , foot1[j-1].delta_y , (j-1)%2 , pathnumber , foot1 , i);
	j = 0;
	check = 1;
	while(j<i-1 && check == 1)
	{//	bot.leg[0]->runIK(390,0,0,0);
	...
	//	bot.updateBot();
		printf("\n\n\nSTEP %d\n\n",j);
		if (foot1[j].delta_y == 0 && foot1[j].delta_x == 0)
			walk.dribble();
		else
			walk.dribble(foot1[j].delta_y/2,foot1[j].delta_x,foot1[j].delta_theta,0);// + pow(-1,i)*4.544584329 ,0);
	 	// walk.dribble();
