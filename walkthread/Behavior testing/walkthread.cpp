...
int footstepmain(double v_initial , double initial_delta_y , int lr , int pathnumber , foot step[] , int & stepcount)
{
	int count =	0,check = 0;
	int count_ref = 0;
...
/*	PathStructure ps; 
	...
	ps.absObstacles[1].x=28 ;
	ps.absObstacles[1].y=39;
...
///////////////////////////////////////
	...
	{
		fil1.open("paths2.dat", ios::in|ios::binary);
	}
...
int main()
{
	...
	footstepmain(10 , foot1[j].delta_y , i%2 , pathnumber , foot1 , i);
	j=0;
	int check = 1;
	while(j<i-1 && check == 1)
	...
	pathnumber = 2; 
	if (check == 0)
		footstepmain(walk.velocity2() , foot1[j].delta_y , i%2 , pathnumber , foot1 , i);
	else
		footstepmain(walk.velocity2() , foot1[j-1].delta_y , i%2 , pathnumber , foot1 , i);
	j = 0;
	check = 1;
	while(j<i-1 && check == 1)
	{//	bot.leg[0]->runIK(390,0,0,0);
