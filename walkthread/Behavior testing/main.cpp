...
int main()
{
	...
	do
	{
		...
		// walkthread::main(Walk::velocity() , foot1[j].y , i%2 , pathnumber , foot1);
		ifstream fin;
		fin.open("Errortest.dat",ios::in|ios::binary);
		while(!fin.eof())
		{
		...
		//	bot.updateBot();
			printf("\n\n\nSTEP %d\n\n",j);
			walk.dribble(foot1[j].y/2,foot1[j].x,foot1[j].ang,0);// + pow(-1,i)*4.544584329 ,0);
		 	// walk.dribble();
			if (!getInput(&key))
				j++;
